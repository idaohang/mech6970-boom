...
% $Revision: 1.1.10.12 $  $Date: 2009/09/03 05:17:52 $
...
for l = 1:numel(fieldnames)
    ...
    if isempty(m)
        ...
        warning(wid, ...
            ['Selector field name ''%s'' doesn''t match a shapefile' ...
            ' attribute name.  Ignoring selector.'],...
            fieldnames{l})
        return;
    end
    position(l)  = m;
...
if isempty(requestedFieldNames)
    ...
    for k = 1:numel(requestedFieldNames)
        ...
        if isempty(index)
            wid = sprintf('%s:%s:nonexistentAttributeName',getcomp,mfilename);
            warning(wid, ...
                ['Attribute name ''%s'' doesn''t match a shapefile', ...
                ' attribute name.  It will be ignored.'], ...
                requestedFieldNames{k})
        end
        for l = 1:numel(index)
