...
%   Land Cover Characteristics (GLCC).  REGION is a string that specifies
%   the geographic coverage of the file.  Valid regions are:
%     'g'  or 'global'
%     'af' or 'africa',
...
%     'na' or 'north america',
%     'sa' or 'south america'.
%   FILENAME is a string specifying the name of the data file. Z is a
%   geolocated data grid with coordinates LATGRAT and LONGRAT in units of
%   degrees. Z, LATGRAT, and LONGRAT have class double.  The coordinates
%   for data that lies in the interrupted area of the projection is set to
%   NaN.  A scale factor of 100 is applied to the original dataset such
...
%
%   For details on locating AVHRR data for download over the Internet, see
%   the following documentation at the MathWorks web site:
%
%   <a href="matlab:
%   web('http://www.mathworks.com/support/tech-notes/2100/2101.html#avhrr')
...
% $Revision: 1.1.6.6 $  $Date: 2007/11/09 20:29:52 $
...
if diff(rlim) <= 0 || diff(clim) <= 0
    ...
    [rIndGrat, cIndGrat, Z] = flatRasterRead(filename, nrows, ncols, ...
        rlim, clim, precision, scalefactor, gsize, mfilename);
...
switch lower(region)
    ...
    otherwise
        error('map:avhrrgoode:invalidRegionString', ...
           ['Valid region strings are\n', ...
            '   ''%s'' or ''%s'',\n', ...
            '   ''%s'' or ''%s'',\n', ...
            '   ''%s'' or ''%s'',\n', ...
            '   ''%s'' or ''%s'',\n', ...
...
assert(1 + diff(xlim)/1000 == ncols ...
    && 1 - diff(ylim)/1000 == nrows, ...
    'map:avhrrgoode:inconsistentDimensions', ...
    'Internal error: Inconsistent spatial dimensions.');
...
filename = checkfilename(filename, {'img'}, mfilename, 2, false);

% Verify SCALEFACTOR.
checkinput(scalefactor, {'numeric'}, {'scalar','positive'}, mfilename, ...
    'SCALEFACTOR',3);
...
% Verify GSIZE.
if ~isempty(gsize) && ~isequal([1 2],size(gsize)) || ~isnumeric(gsize)
    error('map:avhrrgoode:invalidGSIZE', ...
        'GSIZE must be a two element vector or an empty matrix.');
end

% Verify NROWS.
checkinput(nrows, {'numeric'}, {'scalar'}, mfilename,'NROWS', 7);

% Verify NCOLS.
checkinput(ncols, {'numeric'}, {'scalar'}, mfilename,'NCOLS', 8);
...
assert(lonlim(1) <= lonlim(2), ...
    'map:avhrrgoode:lonlimNotAscending', ...
    'Longitude limits must be ascending.');

assert(-180 <= lonlim(1) && lonlim(2) <= 180, ...
    'map:avhrrgoode:lonlimOutsideLimits', ...
    'Longitude limits must be in range [-180 180].');
