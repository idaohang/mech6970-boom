...
%
% STRUC = DCWREAD reads a DCW file and returns file contents in a
...
% Written by:  W. Stumpf
% $Revision: 1.1.6.6 $  $Date: 2007/10/10 20:48:54 $
...
% 	disp('Variable length records, selected')
	...
	if xfid == -1; 
		xfilename(length(xfilename))='x';
		xfid = fopen([filepath xfilename],'r');
		if xfid == -1; error(['map:' mfilename ':mapformatsError'], ['Index file ' xfilename ' not found']); end
	end
	fclose(xfid);
...
% last type byte is reserved.
		...
		otherwise
			...
			error(['map:' mfilename ':mapformatsError'], ['Field type ' field(i).type ' not implemented'])
...
%
		...
		if ismember(i,[selectfields{:}])
			...
			while 1
				...
				[str,count] = fread(fid, [recordlen blocksize], 'char');
				% read all the records and subscript in later
				% can't read adjacent data and then skip
				...
				if count < blocksize
					% must have run into end of file
					break;
				end
			end %while
...
%
% jump to beginning of a desired record and then parse fields
%
