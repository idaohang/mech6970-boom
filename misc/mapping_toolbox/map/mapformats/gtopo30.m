...
%   in the current directory or on the MATLAB path.  If TILENAME is empty
%   or omitted, a file browser will open for interactive selection of the
...
%   within a GTOPO30 CD-ROM or directory structure. The DIRNAME input is a
%   string with the name of the directory which contains the GTOPO30 tile
%   directories or GTOPO30 tiles.  Within the tile directories are the
%   uncompressed data files.  The DIRNAME for CD-ROMs distributed by the
%   USGS is the device name of the CD-ROM drive.  As with the case with a
%   single tile, any cells in the grid specified by LATLIM and LONLIM are
%   NaN filled if they are not covered by a tile within DIRNAME.
%   SAMPLEFACTOR if omitted or empty defaults to 1. LATLIM if omitted or
%   empty defaults to [-90 90]. LONLIM if omitted or empty defaults to
%   [-180 180].
%  
%   For details on locating GTOPO30 data for download over the Internet,
...
% $Revision: 1.1.6.9.12.2 $  $Date: 2011/01/29 14:47:40 $
...
% Validate FILENAME class.
if ~isempty(filename) && ~ischar(filename)
    error('map:gtopo30:invalidType', ...
        ['Function %s expected its first input, %s (or %s),', ...
        'to be type char.\n\nInstead its type was %s.'], ...
        upper(mfilename), 'TILENAME', 'DIRNAME', class(filename));
end
...
if numel(lat) ~= 2
    error('map:gtopo30:latlonSizeMismatch',  ...
        'Function %s expected its %s input argument, %s, to be a vector of length 2.',  ...
        upper(mfilename), num2ordinal(lat_pos), lat_var_name );
end

if lat(1) > lat(2)
    error('map:gtopo30:latLimitError', ...
        'Latitude limits must be ascending.');
end
...
if numel(lon) ~= 2
    error('map:gtopo30:latlonSizeMismatch',  ...
        'Function %s expected its %s input argument, %s, to be a vector of length 2.',  ...
        upper(mfilename), num2ordinal(lon_pos), lon_var_name );
end
...
if ~exist([tilePathName, '.DEM'], 'file')
    error('map:gtopo30:fileNotFound', ...
         '%s file, "%s.DEM", does not exist.', 'GTOPO30', tilePathName);
end
...
if isempty(tileNames)
    ...
    Z = NaN;
    refvec = calcRefVec(latlim, lonlim, samplefactor);
    warning('map:gtopo30:noDataInDir', ...
        ['The %s directory, "%s", does not contain data for the ' ...
        'specified latitude longitude limits.'], ...
        'GTOPO30', dirname);
else
...
try
    ...
    error('map:gtopo30:nanError', ...
        ['The following error occurred while attempting to create a ', ...
        'data grid of size %d-by-%d:\n%s'],  nrows, ncols, e.message);
end
...
if fid==-1
    error('map:gtopo30:fileNotFound', ...
        'GTOPO30 header file not found: %s', filename)
end
...
if any([isempty(ulxmap), isempty(ulymap), isempty(xdim), isempty(ydim)]) 
    try
        ...
        if isequal(e.identifier,'map:worldfile:expectedSixNumbers')
            error('map:gtopo30:incompleteHeader',  ...
...
if fid==-1
    ...
    fid = fopen(filename,'r');
    if fid==-1
        error('map:gtopo30:ancillaryFileNotFound',  ...
...
if count ~= 6 
    fclose(fid);
    error('map:worldfile:expectedSixNumbers', ...
...
refmat = refvec2mat(refvec2, sizeZ2);
...
function [tileNames, XMIN, XMAX, YMIN, YMAX] = getTileNamesAndLimits
% Return a cell array of the names of all the GTOPO30 tiles and their
% corresponding longitude and latitude limits.

% The number of GTOPO30 tiles:
numTiles = 33;

% Open the GTOPO30 bounding box file (a text file located in
% toolbox/map/mapdisp).
fid = fopen('gtopo30s.dat','r');
if fid == -1
    error('map:gtopo30:internalError', 'Unable to open gtopo30s.dat')
end

% Read names and bounding quad limits.
C = textscan(fid, '%s%d%d%d%d', numTiles);
tileNames = upper(C{1});
YMIN = double(C{2})';
YMAX = double(C{3})';
XMIN = double(C{4})';
...
function tileNames = intersectTilesWithGeoQuad( ...
    latlim, lonlim, tileNames, XMIN, XMAX, YMIN, YMAX)
% Trim the input list of all tile names to match only those that match the
% requested limits. 

if lonlim(1) < lonlim(2)
    % case where dateline is not crossed: lonlim(1) < lonlim(2)
    tf = findMatchingTiles(latlim, lonlim, XMIN, XMAX, YMIN, YMAX);
else
    ...
    tfEast = findMatchingTiles(latlim, lonlim, XMIN, XMAX, YMIN, YMAX);
    
    % do western side of the dateline second
    lonlim(1) = -180; 
    lonlim(2) = lmax;
    tfWest = findMatchingTiles(latlim, lonlim, XMIN, XMAX, YMIN, YMAX);
...
if ~isempty(tf)
    ...
    error('map:gtopo30:invalidLimits', ...
        'No matching files found for the specified limits.');
end
...
if any(index)
    tileLatlim = [YMIN(index) YMAX(index)];
    tileLonlim = [XMIN(index) XMAX(index)];
else
    error('map:gtopo30:noMatchingTile', ...
        'The specified %s, "%s", exists but does not match the name of a known %s tile.', ...
        'TILENAME', tileName, 'GTOPO30');
end
...
if ~limitsContainTile
    files = sprintf('%s\n', tileNames{:});
    warning('map:gtopo30:noData', ...
        ['The GTOPO30 tile, "%s", does not contain data for the ' ...
        'specified latitude longitude limits. Data for the ', ...
        'latitude and longitude limits can be found in tiles: \n%s'], ...
        tileName, files);
end

%--------------------------------------------------------------------------

function tf = findMatchingTiles(latlim, lonlim, XMIN, XMAX, YMIN, YMAX)
