...
% fixed format file.  Filename is a string containing the name of the file.
% If it is empty, the file is selected interactively. filestruct is
...
% specified in the vector recordIDs.  For example, recordIDs = [1 2 4]. All
% of the fields in the selected records are read.
% 
...
% the filestruct name field.
% 
% See also READMTX, SPCREAD, DLMREAD.

% Copyright 1996-2009 The MathWorks, Inc.
% $Revision: 1.1.6.6 $  $Date: 2009/11/16 22:24:59 $
% Written by:  W. Stumpf
...
elseif nargin == 6
	...
	warning('map:readfields:obsoleteSparseOption', ...
        'Sparse option for READFIELDS is obsolete. READFIELDS will correctly read \n formatted files with empty elements')
else
...
case {'sparse','full'}
otherwise
	error(['map:' mfilename ':mapformatsError'], 'population must be ''sparse'' or ''full''')
end
...
if ~filewasopen && mod(eof-datastartpos,recordlen) ~= 0;
	...
	if ~isempty(CRindx) && all( diff(diff(CRindx))==0 )
		...
		fclose(fid);
		error(['map:' mfilename ':mapformatsError'], sprintf(['File size does not match inputs. \n' ...
				...
				for k = 1:field(i).length
					...
					if size(datacol,1) ~= size(strng2,1) 
						...
						if ~isempty(notemptyindx)
							...
							error(['map:' mfilename ':mapformatsError'], ['Data in field ' num2str(i) ' does not match format type'])
						end
					end
...
%
% jump to beginning of a desired record and then read fields
%
	status = fseek(fid,datastartpos+(k-1)*recordlen,-1);
	if ftell(fid) >= eof || status == -1 ; 
		error(['map:' mfilename ':mapformatsError'], 'File does not contain the requested record'); 
	end
...
% Assume a size, and notify user.
		...
		case {	'short',...                     	% integer,  16 bits, 2 bytes
                ...
                warning('map:readfields:twoByteSizeAssumption', ...
                    'Assuming machine dependent %s is 16 bits, 2 bytes long', field(i).type)
				recordlen = recordlen + 2*field(i).length;
				field(i).bytes = 2;
		...
		case {	'int',...            				% integer,  32 bits, 4 bytes
        		...
        		'float'}          					% floating point, 32 bits, 4 bytes
  				...
  				warning('map:readfields:fourByteSizeAssumption', ...
                    'Assuming machine dependent %s is 32 bits, 4 bytes long', field(i).type)
				recordlen = recordlen + 4*field(i).length;
				field(i).bytes = 4;

		case 'double'        						% floating point, 64 bits, 8 bytes
		
  				warning('map:readfields:eightByteSizeAssumption', ...
                    'Assuming machine dependent %s is 64 bits, 8 bytes long', field(i).type)
				recordlen = recordlen + 8*field(i).length;
				field(i).bytes = 8;
		...
		otherwise
			if ~isempty(findstr('%',field(i).type)) % SCANF-style formatted data
				...
				if isempty(numericCharindx) || (  ~isempty(decimalpointindx) && decimalpointindx == 1);
					error(['map:' mfilename ':mapformatsError'], ['Element ' num2str(i) ' type field needs a field width for formatted input. Example: ''%8g'' '])
				end
				...
				fclose(fid)
				error(['map:' mfilename ':mapformatsError'], ['Field type ' field(i).type ' not recognized'])
			end
		end
