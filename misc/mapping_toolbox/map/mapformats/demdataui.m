...
% $Revision: 1.1.6.8 $  $Date: 2009/11/09 16:26:52 $
% Written by:  W. Stumpf
...
try
   ...
   if isempty(liststrs); error(['map:' mfilename ':mapformatsError'], 'No external DEM data found. Click the HELP button for more on external data sources'); end
   ...
   case 'ETOPO5'
      ...
      [map,maplegend] = etopo5read(scalefactor,latlim,lonlim);
      h.extracteddata(end+1).type = 'regular';
...
otherwise
   ...
   extractedsize = sizem(latlim,lonlim,1/(scalefactor*resolutionInDegrees));
...
for i=1:length(sources)
   ...
   case 'SatBath'
      ...
      htemp = patch(x, y, 'y', 'Tag', 'satbathtile');
      zdatam(htemp,-1);restack(htemp,'bottom')
      set(htemp,'facecolor',[0.99          1.00          0.85],'edgecolor',.75*[1 1 1])
...
% files are themselves on the MATLAB path.
% The way this used to work is that the code searched to see if the parent
...
for n = 1:length(s)
    ...
    if exist(dir_name,'dir') || exist(udir_name,'dir')
        ...
        for m = 1:num_of_paths
            ...
            ind = findstr(singlepath,filesep);
            foundpth{end+1} = singlepath(1:ind(end)-1);
        end
    end
end
...
for i=1:length(d)
   ...
   if ~isnan(sgn) && ~isempty(lllon)
      ...
      for j=1:length(dd)
         ...
         [path,name,ext] = fileparts(dd(j).name(2:end));
...
while ~done
   ...
   if ~isempty(answer)
      try
         ...
         h = errordlg(e.message,'DEMDATAUI SAVE ERROR','modal');
      end
   else
      return
   end
...
helpstr = { ...
      ...
      ,[],[],'SAVE BUTTON',[],[ ...
         ...
         'The results are stored in a Mapping Toolbox display structure. ' ...
