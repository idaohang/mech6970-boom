...
% display structure.  It can be displayed with function DISPLAYM or
% converted to a geostruct with function UPDATEGEOSTRUCT. 
%
...
% invalid properties are entered. All properties are specified  as strings.
% Values may be integers, strings, or to match multiple values,  cell
% arrays of integers or strings. 
%
% For details on locating VMAP0 data for download over the Internet,
% see the following documentation at the MathWorks web site: 
%
% <a href="matlab: 
% web('http://www.mathworks.com/support/tech-notes/2100/2101.html#vmap0') 
...
% $Revision: 1.1.6.11 $  $Date: 2009/07/27 20:16:06 $
% Written by:  W. Stumpf
...
if isstr(topolevel)
   ...
   if isempty(topoindx); error(['map:' mfilename ':mapformatsError'], 'Valid topology levels are ''patch'', ''line'', ''point'' or ''text'' ');end
   if nargout ~= 1
      error(['map:' mfilename ':mapformatsError'], 'Number of output arguments doesn''t match requested topology levels')
   end
...
elseif iscell(topolevel)
   ...
   for i=1:length(topolevel)
      if ~isstr(topolevel{i}); error(['map:' mfilename ':mapformatsError'], 'Topology level must be a cell array of strings'); end
      thistopoindx = strmatch(topolevel{i},{'patch','line','point','text'},'exact');
      if isempty(thistopoindx); error(['map:' mfilename ':mapformatsError'], 'Topology level must be ''patch'', ''line'', ''point'' or ''text'' ');end
      topoindx = [topoindx thistopoindx];
   end % for
   
   topoindx = unique(topoindx);
   if length(topoindx) ~=length(topolevel)
      error(['map:' mfilename ':mapformatsError'], 'Redundant requests in topology levels')
   elseif nargout ~= length(topoindx)
      error(['map:' mfilename ':mapformatsError'], 'Number of outputs doesn''t match requested topology levels')
   end
else
   error(['map:' mfilename ':mapformatsError'], 'Topology level must be a string or cell array of strings')
end % if
...
if isempty(strmatch('VMAPLV0',upper(strvcat(dirstruc.name)),'exact'))
   error(['map:' mfilename ':mapformatsError'], ['VMAP Level 0 disk not mounted or incorrect devicename ']); 
end
...
elseif ~isempty(strmatch('vmaplv0',{dirstruc.name},'exact'))
   ...
   error(['map:' mfilename ':mapformatsError'], 'Unexpected mixed case filenames')
end

% check feature table inputs
if (~(isempty(featuretable)))
   if (~ischar(featuretable) & (~iscellstr(featuretable)))
      error(['map:' mfilename ':mapformatsError'], 'Feature Table Inputs must be strings or cell strings')
   end
end
...
if ~isempty(property)
   if (mod(length(property), 2) ~= 0)
      error(['map:' mfilename ':mapformatsError'], 'Properties must be in pairs: { property1 {value[s]} property2 {value[s]} ... }')
   end
   f = 1:2:length(property);
   if ~iscellstr(property(f))
      error(['map:' mfilename ':mapformatsError'], 'Property Names must be Strings')
   end
...
dirstruc = dir(filepath);
if isempty(dirstruc)
   error(['map:' mfilename ':mapformatsError'], ['VMAP Level 0 disk ' upper(library) ' not mounted or incorrect devicename ']); 
end
...
if isempty(strmatch(theme,{CAT.coverage_name},'exact'))
   ...
   error(['map:' mfilename ':mapformatsError'], ['Theme not present in library ' library char(linebreak) char(linebreak) ...
...
for i=1:length(dotiles)
   ...
   tilepath = strrep(tilepath,'\',filesep);
   if ~isempty(tilepath) && tilepath(end) ~= filesep
       tilepath(end+1) = filesep;
   end
   ...
   tilepath = fullfile(themepath, tilepath);
...
if ~all(ismember(prop.name, fieldnames(FT)))
   ...
   error(['map:' mfilename ':mapformatsError'], ['Property not present in theme ' FTfilename char(linebreak) char(linebreak) ...
