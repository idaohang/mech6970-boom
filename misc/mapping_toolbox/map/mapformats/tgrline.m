...
%
%   TGRLINE is obsolete. More recent TIGER/Line data sets are available in 
%   shapefile format and can be imported using SHAPEREAD.
%
%   [CL,PR,SR,RR,H,AL,PL] = TGRLINE(FILENAME) reads a set of 1994
...
%       point landmarks   (PL)
%
%   Each result can be displayed with function DISPLAYM or converted to
%   a geostruct with function UPDATEGEOSTRUCT.
%
%   [CL,PR,SR,RR,H,AL,PL] = TGRLINE(FILENAME,YEAR) reads the TIGER line
%   files in the format from that year.  The layout of TIGER/Line files is
%   updated periodically and filename extensions may change from year to
%   year. Valid years are 1990, 1992, 1994, 1995, 1999, 2000, 2002, 2003
%   and 2004.
%
%   [CL,PR,SR,RR,H,AL,PL] = TGRLINE(FILENAME,YEAR,COUNTYNAME) uses the
%   string COUNTYNAME to tag the county data.
%
%   For details on locating TIGER/Line data for download over the Internet,
%   see the following documentation at the MathWorks web site:
%
%   <a href="matlab:
%   web('http://www.mathworks.com/support/tech-notes/2100/2101.html#tiger')
%   ">http://www.mathworks.com/support/tech-notes/2100/2101.html</a>

% Copyright 1996-2009 The MathWorks, Inc.
% $Revision: 1.1.6.8 $  $Date: 2009/05/14 17:06:42 $
% Written by:  A. Kim
...
switch year
    ...
    case 1990, ext = '.F4';
    otherwise,
        eid = sprintf('%s:%s:invalidTigerVersion',getcomp,mfilename);
        msg = sprintf('%d is an invalid TIGER/Line version.',year);
        error(eid,'%s',msg)
end
...
[rt1mat,rt_1cl,rt_1a1,rt_1a2,rt_1a3,rt_1b] = readrt1(filename);
...
function rt_i = readrti(filename,year)
...
fid = fopen(filename);
if fid==-1
    error(['map:' mfilename ':mapformatsError'], 'TIGER/Line Record Type I file not found')
end
...
while ~eof
    ...
    if strcmp(lower(t),rt)
        ...
        rt_i(line,1) = str2double(tlid);

        if isempty(find(~isspace(polyidl)))
            rt_i(line,2) = nan;
        else
            rt_i(line,2) = str2double(polyidl);
        end

        if isempty(find(~isspace(polyidr)))
            rt_i(line,3) = nan;
        else
            rt_i(line,3) = str2double(polyidr);
        end
...
if nargin~=1;  error(['map:' mfilename ':mapformatsError'], 'Incorrect number of arguments');  end
...
fid = fopen(filename);
if fid==-1
    error(['map:' mfilename ':mapformatsError'], 'TIGER/Line Record Type 8 file not found')
end
...
while ~eof
    ...
    if strcmp(lower(t),rt)
        ...
        rt_8(line,1) = str2double(t(16:25));

        rt_8(line,2) = str2double(t(26:35));
...
fid = fopen(filename);
if fid==-1
    error(['map:' mfilename ':mapformatsError'], 'TIGER/Line Record Type 7 file not found')
end
...
while ~eof
    ...
    if strcmp(lower(t),rt)
        ...
        if isempty(find(~isspace(t(55:64))))
            ...
            rt_7(land,1).lalong = fixnum(str2double(t(55:64)));
            rt_7(land,1).lalat = fixnum(str2double(t(65:73)));
        end
...
fid = fopen(filename);
if fid==-1
    error(['map:' mfilename ':mapformatsError'], 'TIGER/Line Record Type 2 file not found')
end
...
while ~eof
    ...
    if strcmp(lower(t),rt)
        ...
        rt_2(line,1) = str2double(t(6:15));

        rt_2(line,2) = str2double(t(16:18));

        long(1) = fixnum(str2double(t(19:28)));
        long(2) = fixnum(str2double(t(38:47)));
        long(3) = fixnum(str2double(t(57:66)));
        long(4) = fixnum(str2double(t(76:85)));
        long(5) = fixnum(str2double(t(95:104)));
        long(6) = fixnum(str2double(t(114:123)));
        long(7) = fixnum(str2double(t(133:142)));
        long(8) = fixnum(str2double(t(152:161)));
        long(9) = fixnum(str2double(t(171:180)));
        long(10) = fixnum(str2double(t(190:199)));
        long_2{line,1} = long(find(long));

        lat(1) = fixnum(str2double(t(29:37)));
        lat(2) = fixnum(str2double(t(48:56)));
        lat(3) = fixnum(str2double(t(67:75)));
        lat(4) = fixnum(str2double(t(86:94)));
        lat(5) = fixnum(str2double(t(105:113)));
        lat(6) = fixnum(str2double(t(124:132)));
        lat(7) = fixnum(str2double(t(143:151)));
        lat(8) = fixnum(str2double(t(162:170)));
        lat(9) = fixnum(str2double(t(181:189)));
        lat(10) = fixnum(str2double(t(200:208)));
...
fid = fopen(filename);
if fid==-1
    error(['map:' mfilename ':mapformatsError'], 'TIGER/Line Record Type 1 file not found')
end
...
while ~eof
    ...
    if strcmp(lower(t),rt)
        ...
        if strcmp(lower(t(16)),'1')
            ...
            rt_1cl(c1,1).tlid = str2double(t(6:15));
            ...
            rt_1cl(c1,1).frlong = fixnum(str2double(t(191:200)));
            rt_1cl(c1,1).frlat = fixnum(str2double(t(201:209)));
            rt_1cl(c1,1).tolong = fixnum(str2double(t(210:219)));
            rt_1cl(c1,1).tolat = fixnum(str2double(t(220:228)));
        ...
        if strcmp(lower(t(56:57)),'a1')
            ...
            rt_1a1(c2,1).tlid = str2double(t(6:15));
            ...
            rt_1a1(c2,1).frlong = fixnum(str2double(t(191:200)));
            rt_1a1(c2,1).frlat = fixnum(str2double(t(201:209)));
            rt_1a1(c2,1).tolong = fixnum(str2double(t(210:219)));
            rt_1a1(c2,1).tolat = fixnum(str2double(t(220:228)));

        elseif strcmp(lower(t(56:57)),'a2')

            c3 = c3 + 1;

            rt_1a2(c3,1).tlid = str2double(t(6:15));
            ...
            rt_1a2(c3,1).frlong = fixnum(str2double(t(191:200)));
            rt_1a2(c3,1).frlat = fixnum(str2double(t(201:209)));
            rt_1a2(c3,1).tolong = fixnum(str2double(t(210:219)));
            rt_1a2(c3,1).tolat = fixnum(str2double(t(220:228)));

        elseif strcmp(lower(t(56:57)),'a3')

            c4 = c4 + 1;

            rt_1a3(c4,1).tlid = str2double(t(6:15));
            ...
            rt_1a3(c4,1).frlong = fixnum(str2double(t(191:200)));
            rt_1a3(c4,1).frlat = fixnum(str2double(t(201:209)));
            rt_1a3(c4,1).tolong = fixnum(str2double(t(210:219)));
            rt_1a3(c4,1).tolat = fixnum(str2double(t(220:228)));

        elseif strcmp(lower(t(56)),'b')

            c5 = c5 + 1;

            rt_1b(c5,1).tlid = str2double(t(6:15));
            ...
            rt_1b(c5,1).frlong = fixnum(str2double(t(191:200)));
            rt_1b(c5,1).frlat = fixnum(str2double(t(201:209)));
            rt_1b(c5,1).tolong = fixnum(str2double(t(210:219)));
            rt_1b(c5,1).tolat = fixnum(str2double(t(220:228)));

        elseif strcmp(lower(t(56)),'h')

            c6 = c6 + 1;

            rt_1h(c6,1).tlid = str2double(t(6:15));
            ...
            rt_1h(c6,1).frlong = fixnum(str2double(t(191:200)));
            rt_1h(c6,1).frlat = fixnum(str2double(t(201:209)));
            rt_1h(c6,1).tolong = fixnum(str2double(t(210:219)));
            rt_1h(c6,1).tolat = fixnum(str2double(t(220:228)));

        end

        rt1mat(line,1) = str2double(t(6:15));
        rt1mat(line,2) = fixnum(str2double(t(201:209)));
        rt1mat(line,3) = fixnum(str2double(t(191:200)));
        rt1mat(line,4) = fixnum(str2double(t(220:228)));
        rt1mat(line,5) = fixnum(str2double(t(210:219)));
...
if nargin~=2;  error(['map:' mfilename ':mapformatsError'], 'Incorrect number of arguments');  end
...
while ~done
    ...
    elseif (length(indx1)+length(indx2))==5
        ...
        %
        % 		newlim
        % 		error(['map:' mfilename ':mapformatsError'], 'case error')
...
checknargin(7,7,nargin,fullfile(mfilename,'landpoly'));
if nargin~=7;  error(['map:' mfilename ':mapformatsError'], 'Incorrect number of arguments');  end

polyid = rt_8((rt_8(:,2)==land),1);	% list of polygon id #'s for this landmark

lat = [];
long = [];
for n=1:length(polyid)										% loop through each polygon

    indxl = (rt_i(:,2)==polyid(n));
    indxr = (rt_i(:,3)==polyid(n));
