...
% screen and returns a Mapping Toolbox display structure with the
% matches as text entries.  This structure contains the latitude,
% longitude and theme.  It can be displayed with function DISPLAYM or
...
% $Revision: 1.1.6.6 $  $Date: 2008/06/16 16:47:24 $
% Written by:  W. Stumpf
...
if isempty(strmatch('DCW',upper(strvcat(dirstruc.name)),'exact'))
	error(['map:' mfilename ':mapformatsError'], ['DCW disk ' upper(diskname) ' not mounted or incorrect devicename ']); 
end
...
if strcmp(headerstring(1),';') + strcmp(headerstring(length(headerstring)),';') ~=2
	fclose(fid);
	error(['map:' mfilename ':mapformatsError'], 'File is not in General DCW format')
end
...
if isempty(varlen)
	...
	error(['map:' mfilename ':mapformatsError'], 'Error reading Gazette file: variable length records')
end
...
for i=1:length(field)
	...
	case 'X'
	otherwise
		error(['map:' mfilename ':mapformatsError'], ['Can''t read field type ' field(i).type])
	end %switch
end % for i
...
%
		...
		otherwise
			...
			error(['map:' mfilename ':mapformatsError'], ['Unexpected field type ' field(i).type ' in Gazette File'])
