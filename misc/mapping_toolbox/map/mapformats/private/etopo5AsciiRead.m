...
% $Revision: 1.1.6.1 $  $Date: 2009/09/28 20:24:48 $
...
if all(latlim == [-90 90]) && (all(lonlim == [-180 180]) ...
    ...
    if mod(nrows, sf) ~=0 || mod(ncols, sf) ~=0
        ...
        error(eid, ...
            'SAMPLEFACTOR must divide evenly into %d rows and %d columns.', ...
            nrows, ncols)
    end
...
if ~isempty(rowindx)
    ...
    if fid==-1
        eid = sprintf('map:%s:%sFileNotFound', mfilename, hemisphere);
        error(eid, 'Filename "%s" not found',filename);
    end
    y = srow(rowindx);
    new_m = length(y);
    for m=new_m:-1:1
