...
% $Revision: 1.1.6.2 $  $Date: 2009/03/30 23:39:30 $
...
if (k >= maxIteration)
    error('map:goodeih:fwdConverge', ...
        'The Goode forward iteration failed to converge.');
end
...
breakpt = R * degtorad(dms2degrees([40, 44, 11.8]));
...
for k=1:numel(index)
    ...
    switch region
        case 0
            indx = lon < -pi | lon > degtorad(-40);

        case 1
            indx = lon < -pi | lon > degtorad(-40);

        case 2
            indx = lon < degtorad(-40) | lon > pi;

        case 3
            indx = lon < degtorad(-40) | lon > pi;

        case 4
            indx = lon < -pi | lon > degtorad(-100);

        case 5
            indx = lon < degtorad(-100) | lon > degtorad(-20);
        ...
        case 8
            indx = lon < degtorad(-20) | lon > degtorad(80);

        case 9
            indx = lon < degtorad(80) | lon > pi;
...
% -------------------------------------------------------
lon_center(1) = degtorad(-100);
lon_center(2) = degtorad(-100);
lon_center(3) = degtorad(30);
lon_center(4) = degtorad(30);
lon_center(5) = degtorad(-160);
lon_center(6) = degtorad(-60);
lon_center(7) = degtorad(-160);
lon_center(8) = degtorad(-60);
lon_center(9) = degtorad(20);
lon_center(10) = degtorad(140);
lon_center(11) = degtorad(20);
lon_center(12) = degtorad(140);

% Initialize false eastings for each of the 12 regions
% ----------------------------------------------------
feast(1) = R * degtorad(-100);
feast(2) = R * degtorad(-100);
feast(3) = R * degtorad(30);
feast(4) = R * degtorad(30);
feast(5) = R * degtorad(-160);
feast(6) = R * degtorad(-60);
feast(7) = R * degtorad(-160);
feast(8) = R * degtorad(-60);
feast(9) = R * degtorad(20);
feast(10) = R * degtorad(140);
feast(11) = R * degtorad(20);
feast(12) = R * degtorad(140);
