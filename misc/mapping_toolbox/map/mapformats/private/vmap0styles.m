...
% VMAP0STYLES Set graphic properties of VMAP0 display structures. 

%  Copyright 1996-2009 The MathWorks, Inc.
%  $Revision: 1.1.6.5 $ $Date: 2009/07/06 20:36:31 $ 
...
if ~isstruct(s);
   error(['map:' mfilename ':mapformatsError'], ...
       'Input must be a display structure from VMAP0DATA')
end
...
% save the current random number generator and create a new
% random number generator to get repeatable results

savedDflt = RandStream.setDefaultStream(RandStream('swb2712','seed',0));
...
for i=1:length(tags)
   ...
   if    ~isempty(findstr('Administrative Area',tag)) | ...
      ...
      tag(indx(2):end) = [];
      tag(1:indx(1)+1) = [];
      cdata(i) = floor( 500*sum(cumsum(double(tag))) / ...
                        sum(cumsum(127+0*double(tag))) ); 
...
otherwise
   error(['map:' mfilename ':mapformatsError'], 'unrecognized configuration')
end
...
if strmatch(lower(location),'center')
	...
	if isnumeric(location)
		...
		error(['map:' mfilename ':mapformatsError'], 'Incorrect location input')
	end
end
