...
% $Revision: 1.1.6.8 $ $Date: 2007/12/10 21:39:31 $

error(nargoutchk(1,2, nargout,'struct'));
error(nargchk(1,1,nargin,'struct'));

if ~ischar(filename)
    error(['map:' mfilename ':invalidFilename'], ...
...
% Call SDTSIFC and returned both the INFO structure and the
% DEM data.
...
if isempty(Z)
    ...
    error(['map:' mfilename ':invalidSDTSFile'], ...
        '%s%s is not an SDTS DEM file.',fn, xtn)
end
