...
%   degrees. A scale factor of 100 is applied to the original dataset such
...
% $Revision: 1.1.6.9 $  $Date: 2009/10/16 05:00:34 $
...
if invalidImageLimits || invalidFrameLimits
    ...
    [rIndGrat, cIndGrat, Z] = flatRasterRead(filename, ...
        nrows, ncols, rlim, clim, precision, scalefactor, gsize, mfilename);
...
if ~isempty(gsize) && ~isequal([1 2],size(gsize)) && ~isnumeric(gsize)
    error('map:avhrrlambert:invalidGSIZE', ...
...
assert(-180 <= lonlim(1) && lonlim(2) <= 180, ...
    'map:avhrrlambert:lonlimOutsideLimits', ...
...
switch lower(region)
    ...
    otherwise
        error('map:avhrrlambert:invalidRegionString',...
...
[latlim, lonlim, outputXlim, outputYlim] = ...
    computeImageBBox(mstruct, latlim, lonlim, data, xlim, ylim);

% Compute the referencing matrix.
R = makerefmat(xlim(1), ylim(2), xpercol, yperrow);

%--------------------------------------------------------------------------

function [latlim, lonlim, outputXlim, outputYlim] = ...
    computeImageBBox(mstruct, latlim, lonlim, data, xlim, ylim)
% Compute the bounding box for the image in geographic and map coordinates.
