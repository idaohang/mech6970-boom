...
%
%   [X, CMAP, R] = GEOTIFFREAD(FILENAME) reads a indexed image into X, the
%   associated colormap into CMAP, and constructs a spatial referencing
%   object, R. Colormap values in the image file are automatically rescaled
%   into the range [0,1].
%
...
%   IMFINFO function.
%
%   R is a spatialref.GeoRasterReference object if the image or data grid
%   is referenced to a geographic coordinate system or a
%   spatialref.MapRasterReference object if it is referenced to a projected
%   coordinate system.
%
%   [A, REFMAT, BBOX] = GEOTIFFREAD(FILENAME) reads a georeferenced
%   grayscale, RGB, or multispectral image or data grid into A, the
%   referencing matrix into REFMAT, and the bounding box into BBOX.
%
%   [X, CMAP, REFMAT, BBOX] = GEOTIFFREAD(FILENAME) reads a indexed image
...
% $Revision: 1.1.8.12 $  $Date: 2010/11/17 11:25:16 $
...
extensions = {'tif', 'tiff'};
[filename, url] = checkfilename(filename, extensions, mfilename, 1, true);

% Check and set the image index number.
if ~isempty(inputs)
  idx = inputs{1};
  attributes = {'real' 'scalar' 'positive'};
  checkinput(idx, {'numeric'}, attributes, mfilename, 'IDX', 2);
else
  idx = 1;
end
