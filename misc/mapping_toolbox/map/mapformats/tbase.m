...
%
%  For details on locating TBASE data for download over the Internet, see
%  the following documentation at the MathWorks web site:
%
%  <a href="matlab:
%  web('http://www.mathworks.com/support/tech-notes/2100/2101.html#terrainbase')
%  ">http://www.mathworks.com/support/tech-notes/2100/2101.html</a>
%
%  See also GTOPO30, ETOPO, USGSDEM.

% Copyright 1996-2007 The MathWorks, Inc.
% $Revision: 1.1.6.7 $  $Date: 2007/11/09 20:29:55 $
...
if ~subset
    ...
    assert(mod(1080, sf) == 0 && mod(4320, sf) == 0, ...
        ['map:' mfilename ':mapformatsError'], ...
        'SAMPLEFACTOR must divide evenly into 1080 rows and 4320 columns.')
else
    ...
    shift = lonlim(2)>180;
    if shift
        assert(0 <= lonlim(1) && lonlim(2) <= 360, ...
            ['map:' mfilename ':mapformatsError'], ...
            'lonlim must be between 0 and 360')
    else
        assert(-180 <= lonlim(1) && lonlim(2) <= 180, ...
            ['map:' mfilename ':mapformatsError'], ...
            'lonlim must be between -180 and 180')
    end
...
if fid==-1
    error(['map:' mfilename ':mapformatsError'], 'tbase.bin file not found')
end
if ~subset
    firstrow = 0;
