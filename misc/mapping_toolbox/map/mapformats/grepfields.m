...
% $Revision: 1.1.6.4 $  $Date: 2008/06/16 16:47:26 $
% Written by:  W. Stumpf
...
if nargin >= 5 % file structure specified
    ...
    if ~field(startcol).ischar;
        error(['map:' mfilename ':mapformatsError'], 'Searched field should contain character data')
    end
    if startcol ~= 1
    ...
    if status == -1; error(['map:' mfilename ':mapformatsError'], ferror(['map:' mfilename ':mapformatsError'], fid)); end
...
if ~ischar(searchstring)
    error(['map:' mfilename ':mapformatsError'], 'Search string must be a character')
end
...
switch lower(casesensitivity)
    case {'none','matchcase'}
    otherwise
        error(['map:' mfilename ':mapformatsError'], 'Case sensitivity must be ''none'' or ''matchcase''')
end
...
if rem(nrows,1) ~= 0;
    ...
    fclose(fid);
    error(['map:' mfilename ':mapformatsError'], sprintf([ ...
...
for i=1:length(field)
    ...
    switch field(i).type
        ...
        case {	'short',...                     	% integer,  16 bits, 2 bytes
            ...
            warning('map:grepfields:twoByteSizeAssumption', ...
        ...
        case {	'int',...            				% integer,  32 bits, 4 bytes
            ...
            warning('map:grepfields:fourByteSizeAssumption', ...
        ...
        case 'double'        						% floating point, 64 bits, 8 bytes
            ...
            warning('map:grepfields:eightByteSizeAssumption', ...
