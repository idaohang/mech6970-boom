...
if fid == -1 ;	error(['map:' mfilename ':mapformatsError'], 'Couldn''t open file');end
...
function sgn = readplusminus(t)
   ...
   ch     = sscanf(t,'%1s',1);
	...
	elseif isempty(ch)
		...
		error(['map:' mfilename ':mapformatsError'], 'error reading file: character was not a plus or a minus or empty')
	end
