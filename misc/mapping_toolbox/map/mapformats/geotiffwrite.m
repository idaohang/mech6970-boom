...
%   A, spatially referenced by R, into the output file, FILENAME.
%
%   Use the character string FILENAME to specify the output file name and
%   location. If your FILENAME includes an extension, it must be '.tif' or
%   '.TIF'. The output file is a tiled GeoTIFF file provided that the
%   input, A, is at least 160-by-160 in size; otherwise, it is organized as
%   rows-per-strip.
%
%   A can be an M-by-N array (grayscale image or data grid) or an
%   M-by-N-by-P array (color or hyperspectral image, or data grid). The
%   coordinates of A are geographic and in the 'WGS 84' coordinate system
%   unless you specify either of the parameters 'GeoKeyDirectoryTag' or
%   'CoordRefSysCode' and indicate a different coordinate system.
%
%   In this first syntax, R is always a spatialref.GeoRasterReference
%   object, a referencing matrix, or a referencing vector. (If you are
%   working with image coordinates in a projected coordinate system and R
%   is a spatialref.MapRasterReference object or a referencing matrix, set
%   the 'GeoKeyDirectoryTag' or 'CoordRefSysCode' parameter, accordingly.
%   See the Parameter-Value Options section for more information.)
%
%   GEOTIFFWRITE(FILENAME, X, CMAP, R) writes the indexed image in X and
%   its associated colormap, CMAP, to FILENAME.
%
%   GEOTIFFWRITE(...,PARAM1, VAL1, PARAM2, VAL2,...) specifies parameters
%   that control various characteristics of the output file. The parameter
...
%   -----------------------
%   'CoordRefSysCode'        A scalar positive integer-valued number that 
...
%                            system with a string, the 'EPSG:' prefix must
%                            be present. See the GeoTIFF specification or
%                            the EPSG data files (pcs.csv and gcs.csv) for
%                            the code numbers.
%
%                            If you do not specify a value for this
%                            parameter, the default value is 4326,
...
%                            coordinate system, you will receive an error.
%
%   'GeoKeyDirectoryTag'     A structure that specifies the GeoTIFF 
...
%                            case-insensitive. The structure may be
...
%                            GeoTIFFTags.GeoKeyDirectoryTag. If you set
%                            certain fields of the 'GeoKeyDirectoryTag' to
%                            inconsistent settings, you will receive an
%                            error message. See the
%                            <a href="matlab:doc geotiffwrite">geotiffwrite</a> reference page 
%                            for more details about error messages.
%
%   'TiffTags'               A structure that specifies values for the
%                            TIFF tags in the output file. The field names
%                            of the structure match the TIFF tag names that
%                            are supported by the Tiff class.  The field
...
%   <a href="matlab:web('http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.1')
%   ">http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.1</a>
%
%   The 'CoordRefSysCode' value for projected coordinate systems may be
%   found in the GeoTIFF specification at:
%   <a href="matlab:web('http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.3.1')
%   ">http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.3.1</a>
% 
...
%   <a href="matlab:web('http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.2')
%   ">http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.2</a>
%
...
%   demcmap(get(h,'ZData'))
%   
%   See also GEOTIFFINFO, GEOTIFFREAD, IMREAD, IMWRITE, Tiff.

% Copyright 2010 The MathWorks, Inc.
% $Revision: 1.1.6.2.2.1 $  $Date: 2010/12/06 00:01:17 $
...
function [filename, A, cmap, R, Params] = parseInputs( ...
    filename, A, varargin)
...
%   'TiffTags', 'CoordRefSysCode', 'GeoKeyDirectoryTag'
...
switch numDataArgs
    % (FILENAME, A, 'PARAM1', ...)
    case 0
        error('map:geotiffwrite:tooFewDataArgs', ...
            ['Function %s expected at least two input numeric arguments,', ...
            ' ''%s'' and ''%s'', but was called instead with 0 input ', ...
            'numeric arguments.'], 'GEOTIFFWRITE', 'A', 'R')
    ...
    otherwise
        ...
        error('map:geotiffwrite:tooManyDataArgs', ...
            ['Function %s expected at most three input numeric ', ...
            'arguments, ''%s'', ''%s'', and ''%s'', but was called ', ...
            'instead with %d input numeric arguments.'], ...
            'GEOTIFFWRITE', 'A', 'CMAP', 'R', numDataArgs + 2)
end
...
assert(mod(numel(inputs), 2) == 0, ...
    'map:geotiffwrite:parameterCountNotEven', ...
    'Optional name-value inputs must be provided in pairs.');

% Create output structure with paramNames as the field names of the
% structure. The field values are all [].
paramNames = {'TiffTags', 'CoordRefSysCode', 'GeoKeyDirectoryTag'};
...
assert(strcmpi(ext, tifExt) || isempty(ext), ...
    'map:geotiffwrite:invalidExtension', ...
    ['Function %s expected the file extension of ''%s'' ', ...
    'to be ''%s'' or empty.'], upper(mfilename), filename, tifExt);
...
if isempty(cmap)
    ...
    numRows = size(cmap,1);
    assert(numRows <= maxNumColors, ...
        'map:geotiffwrite:tooManyColormapValue', ...
        'The colormap can have length up to %d, but its actual length is %d.', ...
        maxNumColors, numRows);
...
if isempty(TiffTags)
    ...
    if isfield(TiffTags, 'RowsPerStrip') && ...
         ...
         error('map:geotiffwrite:invalidImageOrganization', ...
             ['The ''%s'' field name, ''%s'', must not be ', ...
             'specified with ''%s'' or ''%s''.'], ...
             'TiffTags', 'RowsPerStrip', 'TileLength', 'TileWidth');
    end
end
...
if ischar(value)
    ...
    if ~any(index)
        ...
        cvalues(end-1:end) = '. ';
        error('map:geotiffwrite:invalidTiffTagValue', ...
            ['Expected %s to match one of the following numeric values: %s', ...
            'The input, %d, did not match any of the valid values.'],  ...
            valueName, cvalues, value)
    end
end
...
if tfProjField && tfGeoField
    ...
    % coordinates. (The coordinate reference system code is validate in the
    % next if block).
    assert(tfModelField && GeoKeyTag.(modelField) == 1, ...
        'map:geotiffwrite:geoAndMapProjections', ...
        'Both ''%s'' and ''%s'' fields have been defined. ', ...
        'Expected the ''%s'' field to be defined and to be set ', ...
        'to the value of %d, indicating projected coordinates.', ...
        modelField, 1);
end
...
if tfProjField
    ...
    geoKeyType = getCoordRefSysCodeType(key);
    % The geoKeyType must be 'projected' or user-defined.
    assert(any(strcmp(geoKeyType, {'projected','user-defined'})), ...
        'map:geotiffwrite:invalidProjectedCSTypeGeoKey', ...
        ['Expected the ''%s'' field value to indicate a ', ...
        'numeric value for a known projected coordinate system.'], ...
        ['GeoKeyDirectoryTag.' projField])
    
    if tfModelField
        % GTModelTypeGeoKey = 1 indicates projected coordinates
        assert(GeoKeyTag.(modelField) == 1, ...
            'map:geotiffwrite:invalidProjectedModelType', ...
            ['Expected the ''%s'' field value to be 1 to indicate a ', ...
            'projected coordinate system.'], ...
            ['GeoKeyDirectoryTag.' modelField])
    else
...
elseif tfGeoField
    ...
    assert(any(strcmp(geoKeyType, {'geographic', 'user-defined'})), ...
        'map:geotiffwrite:invalidGeographicTypeGeoKey', ...
        ['Expected the ''%s'' field value to indicate a ', ...
        'numeric value for a known geographic coordinate system.'], ...
        ['GeoKeyDirectoryTag.' geoField])
    
    if tfModelField
        % GTModelTypeGeoKey = 2 indicates geographic coordinates
        assert(GeoKeyTag.(modelField) == 2, ...
            'map:geotiffwrite:invalidGeographicModelType', ...
            ['Expected the ''%s'' field value to be 2 to indicate a ', ...
...
% Return the type of coordinate reference system. CODE is a numeric value.
% TYPE is returned as a string with one of the following values:
...
if isempty(geographicCodes)
   filename = fullfile(epsgdir,'gcs.csv');
   geographicCodes = readcsv(filename);
   % mlock
end
...
if isempty(projectedCodes)
    filename = fullfile(epsgdir,'pcs.csv');
    projectedCodes = readcsv(filename);
    % mlock
end
...
function  dirname = epsgdir
% Return the directory to the EPSG data.
...
% <pathstr>/projdata/epsg_csv
dirname  = fullfile(pathstr,'projdata','epsg_csv');

%--------------------------------------------------------------------------

function codes = readcsv(csvfile)
% Read a CSV file and return the code numbers as an array.

fid = fopen(csvfile);
c = textscan(fid,'%f%s%s%s%f%f%f%f%f%f%s%s%s%s%s%s%s%s%s', ...
...
if ~isempty(coordRefSysCode)   
    ...
    if ischar(coordRefSysCode)
        ...
        epsg = 'EPSG:';
        assert(strncmpi(value, epsg, numel(epsg)), ...
            'map:geotiffwrite:notEPSG', ...
            ['Expected the ''%s'' input to contain the string prefix,' ...
            ' ''%s''.'], strName, epsg)
    ...
    coordRefSysCodeType = getCoordRefSysCodeType(coordRefSysCode);
    assert(strcmp(coordRefSysCodeType, 'projected') ...
        || strcmp(coordRefSysCodeType, 'geographic'), ...
        'map:geotiffwrite:invalidCoordRefSysCodeType', ...
        ['The ''%s'' parameter must indicate a geographic or projected', ...
        ' coordinate reference system.'], 'CoordRefSysCode');
end
...
if ~isempty(coordRefSysCode)
    ...
    if ~isempty(geoKeyType)
        ...
        % coordinate system.
        assert(strcmp(coordRefSysCodeType, geoKeyType), ...
            'map:geotiffwrite:multipleCoordinateReferenceSystemTypes', ...
            ['Expected the ''%s'' parameter to specify a %s ', ...
            'coordinate reference system.'], 'CoordRefSysCode', geoKeyType);
    end
    % Merge the CoordRefSysCode into the GeoKeyTag structure.
...
if strcmp(coordRefSysType, 'geographic')
    if ~isa(R,'spatialref.MapRasterReference')       
        angle_units = 'degrees';
        R = internal.map.convertToGeoRasterRef(R, rasterSize, angle_units, ...
            upper(mfilename), 'R', arg_pos);
    else
        error('map:geotiffwrite:expectedGeoRasterReference', ...
            ['The input, %s, is a %s object', ...
            ' indicating that you are working in a projected coordinate system.', ...
            ' If so, then specify a projected coordinate system by setting' ...
            ' the appropriate values for the', ...
            ' ''%s'' or ''%s'' optional parameters.'], ...
            'R', 'spatialref.MapRasterReference', ...
            'CoordRefSysCode', 'GeoKeyDirectoryTag');
    end   
...
% Validate and convert R to a spatialref.MapRasterReference object.

if numel(R) == 3
    error('map:convertToMapRasterRef:refvecNotAllowed', ...
        ['The input, %s, cannot be a referencing vector when the', ...
        ' coordinates are projected. It must be either a referencing', ...
        ' matrix or a %s object.'], 'R', 'spatialref.MapRasterReference');
end

R = refmatToMapRasterReference(R, rasterSize, func_name, var_name, arg_pos);

assert(strcmp(R.TransformationType, 'rectilinear'), ...
    'map:convertToMapRasterRef:skewOrRotation', ...
    ['The referencing matrix or object supplied to function %s', ...
    ' specifies that the associated raster is rotated or skewed with', ...
    ' respect to the projected coordinate system.  Function %s', ...
...
if isfield(GeoKeyTag, field)
    assert(rasterType == GeoKeyTag.(field), ...
        'map:geotiffwrite:inconsistentRasterType', ...
        ['Expected the ''%s'' field to be equal to %d, indicating ''%s'' ' ...
        'raster interpretation. Instead it contained a value of %d.'], ...
        ['GeoKeyDirectoryTag.' field], rasterType, ...
        R.RasterInterpretation, GeoKeyTag.(field));
else
...
if isfield(TiffTags, 'Photometric') ...
    ...
    assert(ndims(A) == 3 && size(A,3) == 3, ...
        'map:geotiffwrite:imageSizeNotRGB', ...
        'When the ''%s'' tag is set, the image size must be M-by-N-by3.', ...
...
% Flip the Y direction since the standard GeoTIFF configuration is for 
% columns to run from north-to-south (for a positive scaleY) and the
% Mapping Toolbox configure uses a negative scaleY).
...
function tf = isDoubleParam(value)
...
% Must be scalar-valued and fit within the size of a uint16
tf = ~isUint16 || ~isscalar(value);
...
assert(ndims(A) <= 3, ...
    'map:geotiffwrite:invalidNumberOfDimensions', ...
    'The image size must be M-by-N or M-by-N-by-P.');
...
switch classType
    ...
    otherwise
        error('map:geotiffwrite:UnknownClass', ...
            'The class type ''%s'' is not supported.', ...
            classType);
end
...
function writeGeoTiffFile(filename, A, TiffTags, GeoTiffTags)
