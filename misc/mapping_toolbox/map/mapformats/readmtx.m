...
% 
%  See also READFIELDS, SPCREAD, DLMREAD.

% Copyright 1996-2009 The MathWorks, Inc.
% $Revision: 1.1.6.6 $  $Date: 2009/11/16 22:25:00 $
...
if isequal(size(readrows), [1 1])
    readrows = [readrows readrows];
end

if isequal(size(readcols), [1 1])
    readcols = [readcols readcols];
end

% Catch a case [ n; n], which really means [n n];

if isequal(size(readrows), [2 1]) && diff(readrows) == 0
    readrows = readrows';
end

if isequal(size(readcols), [2 1]) && diff(readcols) == 0
    readcols = readcols';
end

% Check for proper inputs

if ~isequal(size(readrows), [1 2]) ...
        && ~(min(size(readrows)) == 1 && length(readrows) > 2 ) ...
            && ~isequal(size(readrows), [2 1]) 
	error(['map:' mfilename ':mapformatsError'], ...
        ['readrows must be a vector of the form row number, ', ...
        '[start end], start:step:end or [row1; row2]'])
end

if ~isequal(size(readcols), [1 2]) ...
        && ~(min(size(readcols)) == 1 && max(size(readcols)) > 2 ) ...
            && ~isequal(size(readcols), [2 1]) 
	error(['map:' mfilename ':mapformatsError'], ...
        ['readcols must be a vector of the form column number, ', ...
        '[start end], start:step:end or [col1; col2]'''])
end
...
case {'binary', 'formatted, fixed length'}
	...
	if (expectedfilesize ~= eof)
		fclose(fid);
		error(['map:' mfilename ':mapformatsError'], ...
            ['File size does not match inputs. Expected %s ' ...
            ' bytes, but file size is %s bytes.'], ...
...
if ((rowstep == 1 && colstep == 1) || ... 							    % every value
	(colstep == 1 && isequal(unique(diff(readrows)), [1 1]))) ...  		% runs of data fields, with constant skips in record number
	...
	case 'formatted, fixed length'
		...
		for i=1:nrowread
			...
			if count ~= ncols;
				fclose(fid);
				error(['map:' mfilename ':mapformatsError'], ...
                    ['Incorrect number of values parsed. Expected %s ', ...
                    ' values, but parsed %s.'], ...
                    num2str(ncols) , num2str(count))
			end
		...
		for i=1:nrows
			...
			if ismember(i,readrows)
				...
				if count ~= ncols;
                    ...
                    error(['map:' mfilename ':mapformatsError'], ...
                        ['Incorrect number of values parsed. Expected %s ' , ...
                        ' values, but parsed %s. Use a numeric field width ', ...
                        '(e.g. ''%%12g'') for files with no delimiters between fields.'], ...
                        num2str(ncols), num2str(count))
				end
...
if ~isempty(findstr('%',precision))	&& ...
	(~isempty(findstr('c',precision)) || ~isempty(findstr('s',precision)))
	data = char(data);
end
...
case {	'short',...                     	% integer,  16 bits, 2 bytes
        ...
        warning('map:readmtx:twoByteSizeAssumption', ...
            'Assuming machine dependent %s is 16 bits, 2 bytes long', precision)
		fieldlen = 2;
...
case {	'int',...            				% integer,  32 bits, 4 bytes
        ...
        warning('map:readmtx:fourByteSizeAssumption', ...
            'Assuming machine dependent %s is 32 bits, 4 bytes long', precision)
		fieldlen = 4;

case 'double'        						% floating point, 64 bits, 8 bytes

        warning('map:readmtx:eightByteSizeAssumption', ...
            'Assuming machine dependent %s is 64 bits, 8 bytes long', precision)
		fieldlen = 8;
...
otherwise
	...
	if ~isempty(findstr('%',precision)) % SCANF-style formatted data ('%5d')
		...
		fclose(fid)
		error(['map:' mfilename ':mapformatsError'], ...
            ['Field type ' precision ' not recognized'])
	end
end
