...
%  where FILENAME is a case-insensitive string that specifies the name of
%  the ETOPO file, as referenced in the ETOPO data filenames table above.
%  FILENAME may include the directory name; otherwise, the file must be in
%  the current directory or in a directory on the MATLAB path. 
%
...
%
%  <a href="matlab: 
%  web('http://www.mathworks.com/support/tech-notes/2100/2101.html#etopo2') 
...
% $Revision: 1.1.6.5 $  $Date: 2010/09/24 14:33:56 $
...
assert(all(index), ...
    'map:etopo:expectedStringElements', ...
    'All elements of the cell array input must be a string.');

switch numel(filename)
    case 0
        error('map:etopo:tooFewFiles', 'No input file is specified.')
    ...
    case 2
        ...
        if ~isequal(ext1, ext2) || ~isequal(lower(ext1),'.bat')
            error('map:etopo:invalidExtensions', ...
                'Filenames have an invalid extension. Expecting "%s"', ...
                '".bat"');
        end
        
    otherwise
        error('map:etopo:tooManyFiles', 'Too many input files.')
end
...
assert(all(filesExist), ...
    'map:etopo:fileNotFound', ...
    'ETOPO data file "%s" does not exist.', ...
...
assert(numel(latlim) == 2, ...
    'map:etopo:invalidLatLim', ...
    'LATLIM must be a two element vector in units of degrees.');

assert(numel(lonlim) == 2, ...
    'map:etopo:invalidLonLim', ...
    'LONLIM must be a two element vector in units of degrees.');

assert(latlim(1) <= latlim(2), ...
    'map:etopo:invalidLatOrder', ...
    'Second element of LATLIM must be greater than or equal to first element.');

assert(lonlim(1) <= lonlim(2), ...
    'map:etopo:invalidLonOrder', ...
    'Second element of LONLIM must be greater than or equal to first element.')

assert( -90 <= latlim(1) && latlim(2) <= 90, ...
    'map:etopo:invalidLatLimits', ...
    'LATLIM must be between -90 and 90.')

assert(all(lonlim <= 360), ...
    'map:etopo:invalidZeroTo2PiLonLimits', ...
    'LONLIM must be between 0 and 360.')
...
d = dir(filename);
assert(isscalar(d), ...
    'map:etopo:expectedScalarFromDir', ...
    'Expected %s(%s) to return a scalar structure.', 'DIR', filename);
expNumBytes = getFileSize(format);
actNumBytes = d.bytes;
assert(actNumBytes == expNumBytes, ...
    'map:etopo:unexpectedFileSize', ...
    ['The byte size, %s, of the %s input file, "%s", does not match ', ...
    'the expected file size. The expected file size is %s bytes.'], ...
    num2str(actNumBytes), format.FormatVersion, ...
    filename, num2str(expNumBytes));
...
if any(index)
    ...
    etopoList = sprintf('\n  %s', etopoFilenames{:});
    error('map:etopo:nonSupportedFilename', ...
        ['The %s specified, "%s", does not match any of the ', ...
        'supported %s file names: %s\n', ...
        'Please refer to the tech note for obtaining %s data.'], ...
...
if ~isempty(filename)
    ...
    error('map:etopo:noDataFound',  ...
        'Directory "%s" does not contain ETOPO data.', dataDir);
end
...
error('map:etopo:unsupportedFileName', ...
    ['Reading the data file "%s" is no longer supported. Please refer ', ...
    'to the tech note for obtaining %s data.'], 'new_etopo5.bil', 'ETOPO5');
...
assert(isscalar(dataset), ...
    'map:etopo:nonscalarDataset', ...
    'The HDF5 %s file, "%s", does not contain a scalar dataset.', ...
    format.FormatVersion, filename);

% Validate the dimensions of the data in the file. The data stored in the
% file is transposed.
assert(isequal(dataset.Dims, [format.NumCols format.NumRows]), ...
    'map:etopo:invalidDimensions', ...
    ['The HDF5 %s file, "%s", does not contain a dataset ', ...
...
% Make a GeoRasterReference, R, based on values in the structure, FORMAT.

rasterSize = [format.NumRows, format.NumCols];
rasterInterpretation = 'cells';
angleUnits = 'degrees';

firstCornerLat = format.FirstCornerLat;
firstCornerLon = format.FirstCornerLon;
...
inputR = spatialref.GeoRasterReference( ...
...
% Convert latlim and lonlim to column and row indices.
% columnsRunSouthToNorth and rowsRunWestToEast specify the direction for
% the output data grid.
columnsRunSouthToNorth = false;
rowsRunWestToEast = true;
[R, rows, cols] = internal.map.selectGeoRasterSubsamplingLimits( ...
    inputR, latlim, lonlim, samplefactor, samplefactor, ...
    columnsRunSouthToNorth, rowsRunWestToEast);
    
%--------------------------------------------------------------------------

function refvec = makerefvec(R)
% Create a referencing vector from a GeoRasterReference object, R.

% Expect square pixels:
cellsPerDegreeLon = R.DeltaLonDenominator/R.DeltaLonNumerator;
cellsPerDegreeLat = R.DeltaLatDenominator/R.DeltaLatNumerator;
assert(abs(cellsPerDegreeLon) == abs(cellsPerDegreeLat), ...
    'map:etopo:expectedSquareCells', ...
    ['The requested latitude and longitude limits create a resultant ', ...
    'grid whose cells are not square.']);

% Create the referencing vector.
northLat = R.FirstCornerLat;
westLon = R.FirstCornerLon;
refvec = [cellsPerDegreeLon northLat westLon];
