...
%   
%   For details on locating USGS24KDEM data for download over the Internet,
...
%   delete(demFilename);
%
%   See also DEMDATAUI, USGSDEM, USGSDEMS.

% Copyright 1996-2009 The MathWorks, Inc.
% $Revision: 1.1.6.10 $  $Date: 2009/03/30 23:39:26 $
...
if Astruc.PlanimetricReferenceSystemCode == 0
	...
	error(['map:' mfilename ':mapformatsError'], ...
        'DEM file is in geographic coordinates, not UTM. Try using USGSDEM.')	
elseif Astruc.PlanimetricReferenceSystemCode ~= 1
    fclose(fid)
	error(['map:' mfilename ':mapformatsError'], ...
        'DEM file is not in UTM coordinates.')
end
...
% that are not described in DEM Data User's Guide 5 (1993). 
...
for i=1:nprofiles
	...
	mat(:,i) = 	flipud([NaN([nbot 1]); Bstruc(i).profile; NaN([ntop 1])]);
end
