...
% $Revision: 1.9.4.6 $  $Date: 2010/03/04 16:22:45 $
...
if any([ndims(lat) ndims(lon)] > 2)
    error(['map:' mfilename ':mapdispError'], ...
        'Lat and lon data must not contain pages');

elseif ~isequal(size(lat),size(lon))
    error(['map:' mfilename ':mapdispError'], ...
        'Inconsistent dimensions on LAT and LON arguments');
...
elseif ~isequal(sort(size(origin)),[1 3])
    error(['map:' mfilename ':mapdispError'], ...
        'Origin input must be a 2 or 3 element vector')
end
...
if nargin == 5
       [lat1,lon1] = fromRadians(units,lat1,lon1);
end
