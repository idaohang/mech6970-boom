...
%
%  SEEDM(Z,refvec) creates an interactive tool for encoding
%  regular data grids.  Seeds can be interactively specified and the
%  encoded grid generated.  The encoded grid can then be saved back
%  to the workspace.
%
%  See also COLORM, ENCODEM, GETSEEDS, MAPTRIM.

% Copyright 1996-2009 The MathWorks, Inc.
% $Revision: 1.9.4.5 $  $Date: 2009/05/14 17:06:27 $
...
case 'initialize'    %  Initialize the display
      h = seedinit;
      if any(map(:) == 0);   grid2image(map+1,maplegend)
	     else;               grid2image(map,maplegend)
	  end
      panzoom('on')
...
%  Turn panzoom off
	 ...
	 if ~isempty(nseeds) & ~isempty(seedval)
	       ...
	       error(['map:' mfilename ':mapdispError'], ...
               'Blank number of seeds or value.')
	 end
...
case 'seed'       %  Seed the map
	 ...
	 set(h.figure,'UserData',h)
     panzoom('off')
     if any(h.map(:) == 0);   grid2image(h.map+1,h.maplegend)
	     else;                grid2image(h.map,h.maplegend)
	 end
     seedm('zoomon')   %  Turn panzoom back on
