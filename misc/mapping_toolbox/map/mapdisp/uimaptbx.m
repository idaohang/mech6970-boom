...
% $Revision: 1.22.4.10 $  $Date: 2009/11/09 16:26:49 $
...
case 'init',
   ...
   % Turn off zoom for the axes and children.
   hZoom = zoom(ancestor(ax,'figure'));
...
case 'init',
   ...
   if strcmp(get(SavedStuff.Hndl,'FaceColor'),'flat')  %  Remove cdata indexing of patch
      ...
      elseif cdata > max(clim)
          clrs = cmap(size(cmap,1),:);
      else
          cindx = linspace(min(clim),max(clim),size(cmap,1))';
...
case 'edit',
   ...
   while 1
      ...
      if ~isempty(answer)
         set(get(0,'CurrentFigure'),'CurrentAxes',get(SavedStuff.Hndl,'Parent'))
         try
             set(SavedStuff.Hndl,'String',answer{1});
             set(get(0,'CurrentFigure'),'CurrentAxes',get(gcbo,'Parent'))
             break;
         catch e
             uiwait(errordlg(e.message,'Text Edit Error','modal'))
         end
      else
         break
      end
...
case 'drag',
   ...
   set(SavedStuff.Hndl,'Units','data');
   ...
   % temporarily hide Scribe Axes and its children when text object is dragged  
   set(SavedStuff.OldAxes,'Visible','off')
   set(allchild(SavedStuff.OldAxes),'Visible','off')
...
case 'textplace',
   ...
   % restore Scribe Axes visibility
   set(h,'Visible','on')
   set(allchild(h),'Visible','on')
...
case 'name',
   ...
   while 1
      ...
      if ~isempty(answer)
         try
             set(SavedStuff.Hndl,'Name',answer{1});
             break;
         catch e
             uiwait(errordlg(e.message,'Figure Name Error','modal'))
         end
      else
         break
      end
...
case 'exit'
    ...
    if ishghandle(SavedStuff.Hndl,'text')
