...
%
%  MAPHLP3 enables the help utility for AXESMUI and all of the dialog boxes
%  contained in AXESMUI. It also processes the help callbacks and restores
%  the calling dialog to its original state (pre-help call).

%  Copyright 1996-2008 The MathWorks, Inc.
%  $Revision: 1.14.4.9 $  $Date: 2008/10/02 18:57:16 $
%  Written by:  E. Byrns, E. Brown

%  1 or 2 arguments may be supplied to this function.  The first argument
%  is the action string for the switch yard below. The second argument
%  identifies the calling dialog and is used by CustomInit and CustomDone
%  to properly customize the help utility.

%  The help utility change static text to push buttons and then associates
%  the proper callback with these new buttons.  All other dialog controls
%  are disabled.  Upon termination of the utility, the push buttons are
%  returned to their original style.

%  This help file has a particular feature in that the MapProjection popup
%  menu is not disabled in the help mode.  Instead, this control is used to
%  bring up the description of each map file contained in its online help.
...
%  Initialize the help text object
    ...
    set(h.helpfig,'Visible','on');

	 set(h.fig,'UserData',h)         %  Save the additional help handles
    figure(h.fig)                   %  Activate the original modal dialog

case 'closeHelp'
    % allows user to close help window from the figure menu or 
    % with the close button 
    tag = get(gcbf,'Tag');

    % Delete the help dialog
    delete(gcbf);                    

    % Find figure with this help button
    fig = findobj(0,'Tag',tag);
    h = get(fig,'UserData');

    % Restore the original modal dialog
    if ~isempty(h)
      CustomDone(varargin{2},h)       
      refresh(h.fig)
...
case 'forceclose'
    ...
    if isfield(h,'helpfig') && ishghandle(h.helpfig)
      delete(h.helpfig)               %  Delete the help dialog
    end
...
case 'axesmui'         %  Axesmui Main Dialog
   % updated close request function to allow user to close help window
   % via the figure menu or close button 12022002 LSJ
	set(h.helpfig,'Name','Projection Control Help', ...
    'CloseRequestFcn','maphlp3(''closeHelp'',''axesmui'')')   %  Turn help window on
   set([h.helpfig; h.fig], 'Tag','axesmui')
...
case 'frame'         %  Frame Dialog
    ...
    'CloseRequestFcn','maphlp3(''closeHelp'',''frame'')')   
   set([h.helpfig; h.fig], 'Tag','frame')
...
case 'grid'         %  Grid Dialog
    ...
    'CloseRequestFcn','maphlp3(''closeHelp'',''grid'')')   
   set([h.helpfig; h.fig], 'Tag','grid')
...
FontScaling =  guifactm('fonts');
	...
	set(h.helpfig,'Name','Map Labels Control Help',...
                 'CloseRequestFcn','maphlp3(''closeHelp'',''labels'')', ...
	              'Units','Points',  'Position',PixelFactor*72*[0.75 0.12 7 1.8])
   set([h.helpfig; h.fig], 'Tag','labels')
...
case 'labelsetting'         %  Label Setting Dialog
	set(h.helpfig,'Name','Label Settings Help', ...
                'CloseRequestFcn','maphlp3(''closeHelp'',''labelsetting'')');
   set([h.helpfig; h.fig], 'Tag','labelsetting')
...
%  GUI figure windows and fonts look OK across all platforms
	...
	set(h.helpfig,'Name','Grid Settings Help',...
                 'CloseRequestFcn','maphlp3(''closeHelp'',''gridsetting'')', ...
	              'Units','Points',  'Position',PixelFactor*72*[0.75 0.05 7 1.5])
   set([h.helpfig; h.fig], 'Tag','gridsetting')
