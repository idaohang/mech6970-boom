...
% $Revision: 1.10.4.11 $  $Date: 2010/03/22 03:52:09 $
...
elseif nargin == 1 && ~strcmpi(varargin{1},'reset')
    [~,lcolor,~,err] = colstyle(varargin{1});
    error(err) %#ok<ERTAG>
...
zFrame = zeros(size(xFrame)) + min(get(gca,'zlim')) - 1;
