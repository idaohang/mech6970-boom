...
%  OFF).  N,S,E,W Keys move the marker North, South, East or West by 10.0
%  degrees for each keystroke.  n,s,e,w keys move the marker in the
...
% $Revision: 1.10.4.8 $  $Date: 2009/04/15 23:17:09 $
...
case 'on'          %  Turn origin marker on
    ...
    % Find a valid map axes
    axesHndls = findall(gcf,'type','axes');
    mstruct = [];
    for j=1:numel(axesHndls)
       if ismap(axesHndls(j))
         mstruct = gcm(axesHndls(j));
         set(gcf,'currentaxes',axesHndls(j));
         break
       end
    end
    if isempty(mstruct)
...
case 'toggle'         %  Toggle origin marker visible and invisible
	   ...
	   if isempty(hmark)           %  Initial call.  No marker exists
            if ~ismap
                error(['map:' mfilename ':notAMapAxes'], ...
                    'Not a valid map axes')
            end
	        originui('on')
...
case 'key'             %  Key strokes recognized
      ...
      switch keychar
	     case 13,     originui('apply')                    %  Enter/Return key
 ...
 case 'apply'      %  Reproject the map
     ...
     setm(gca,'Origin',[roundn([lat lon], -2) oldor(3)] );
     originui('on');
...
case 'keyorigin'             %  Move origin by keystroke
	 ...
	 if strcmp(direction,'ew')
	     ...
	     lon = fromDegrees(units,lon);
	 else
	     lat = toDegrees(units,lat);
	     lat = round( (lat+delta)/abs(delta)) * abs(delta);
	     lat = fromDegrees(units,lat);
	 end

     [x,y] = mfwdtran(mstruct,lat,lon,0,'none');
