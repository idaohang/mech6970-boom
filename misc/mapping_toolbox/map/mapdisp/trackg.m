...
% $Revision: 1.13.4.8 $  $Date: 2009/03/30 23:39:18 $
...
elseif max(size(ntrak)) ~= 1
    error(['map:' mfilename ':mapdispError'], ...
        'Number of tracks must be a scalar')
end
...
for i = 1:real(ntrak)
       ...
       if isempty(latpts)
           warning('map:trackg:emptyLATPTS','Wrong view for TRACKG');
           return
       end
...
case 'close'	
	...
	% delete the objects
	if ishghandle(s.hstart); delete(s.hstart); end
