...
% $Revision: 1.17.4.9 $  $Date: 2009/05/14 17:06:23 $
    ...
    if ~isempty(x)
        ...
        if ~isempty(ax)
            h0(nPatches+1) = patch(...
                ...
                'UserData', userdata, 'ButtonDownFcn', 'uimaptbx', 'Parent',ax);
            nPatches = nPatches + 1;
        else
            h0(nPatches+1) = patch(...
                'Faces', faces, 'Vertices', [x y zout], 'FaceColor', cdata, ...
