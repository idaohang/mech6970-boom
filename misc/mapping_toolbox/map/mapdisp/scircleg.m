...
% $Revision: 1.14.4.8 $  $Date: 2009/03/30 23:39:12 $
...
elseif ischar(ncirc)
    error(['map:' mfilename ':mapdispError'], ...
        'Number of circles not specified.')
elseif max(size(ncirc)) > 1
    error(['map:' mfilename ':mapdispError'], ...
        'Number of small circles must be a scalar')
end
...
if isempty(str)
    ...
    if length(indx) ~= 1
        error(['map:' mfilename ':mapdispError'], ...
            'Unrecognized track string')
    else
        str = deblank(validstr(indx,:));
    end
end
...
for i = 1:real(ncirc)
       ...
       if isempty(latpts)
           warning('map:scircleg:emptyLATPTS','Wrong view for SCIRCLEG');
           return
       end
...
% 	s.rad = distance(s.trk,clat,s.clon,s.rlat,s.rlon);
	...
	switch units
		...
		case 'Radians'
			ndist = num2str(degtorad(distance(s.trk,clat,s.clon,s.rlat,s.rlon)));
	end
...
% 	s.rad = distance(s.trk,clat,s.clon,s.rlat,s.rlon);
	...
	switch units
		...
		case 'Radians'
			ndist = num2str(degtorad(distance(s.trk,s.clat,s.clon,s.rlat,s.rlon)));
	end
	set(findobj(par,'tag','rad'),'string',ndist);
	% center point
	hcenters = findobj(gca,'tag','InteractiveCircleCenter');
...
case 'changeradius'
	...
	if isempty(str2num(get(gcbo,'string'))) || str2num(get(gcbo,'string')) <=0
		...
		switch units
			...
			case 'Radians'
				ndist = num2str(degtorad(distance(s.trk,s.lat,s.lon,s.rlat,s.rlon)));
		end
...
case 'changeunits'	
	...
	switch units
		...
		case 'Radians'
			ndist = num2str(degtorad(distance(s.trk,lat,lon,rlat,rlon)));
	end
	set(findobj(par,'tag','rad'),'string',ndist);
...
case 'close'
	...
	if ishghandle(s.hcirc)
		s.parent = get(s.hcirc,'parent');	
		if ishghandle(s.parent)
			axes(s.parent);
		end
	end
	% delete the objects
	if ishghandle(s.hcenter); delete(s.hcenter); end
	if ishghandle(s.hend);   delete(s.hend);   end
	if ishghandle(s.hcontrol)
