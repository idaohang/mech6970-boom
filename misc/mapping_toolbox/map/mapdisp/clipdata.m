...
% $Revision: 1.17.4.9 $  $Date: 2010/03/04 16:22:36 $
...
case 'patch'
   ...
   for i = 1:length(indx)
      ...
      case 'old'
         ...
         % encompass the pole was not foolproof. Furthermore, there were
         ...
         %  the data vectors.  This processing must account for the possibility
...
%  is simply inserted into the location of a clip crossing, and thus,
...
%  accidentally push a point to the wrong side of the zero line.
...
%  preceding points or the point is determined by interpolation between
%  the points at the clip location.  For further discussion of this process
%  see the subfunctions rghtfill and leftfill.
...
for j = length(splitvec):-1:1
   ...
   elseif splitvec(j) == length(x)-1 & x(splitvec(j)) < x(splitvec(j)+1)
      ...
      %*******************************************************************************
      %  Clipping between interior points in the vector.  Going right to left.
      %*******************************************************************************
   ...
   elseif x(splitvec(j)) > x(splitvec(j)+1)
      ...
      %*******************************************************************************
      %  Clipping between interior points in the vector.  Going left to right.
      %*******************************************************************************
...
%  (Antarctica is a case like this).  For these patches
...
% patch (interpolated to a minimum spacing to allow for non-uniformly spaced 
...
for i=1:length(cindx)
    ...
    lon1 = lon02pi(cindx(i));
    lon2 = lon02pi(cindx(i) + 1);
    lat1 = lat(cindx(i));
    lat2 = lat(cindx(i) + 1);
    if (lon1 ~= lon2)
        % Use linear interpolation if possible
        lati = lat1 + (lat2 - lat1) * (pi - lon1) / (lon2 - lon1);
    else
        % If the longitudes coincide, just average the latitudes
        lati = (lat1 + lat2) / 2;
    end
    latdateline(i) = lati;
