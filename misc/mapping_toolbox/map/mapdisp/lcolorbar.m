...
% $Revision: 1.4.4.8 $  $Date: 2010/07/19 12:54:01 $
...
if mod(nargin,2) ~= 1
   error('map:lcolorbar:notNameValuePairs', ...
       'Inputs must be labels and property-value pairs.')
end
...
switch loc
    ...
    otherwise
        error('map:lcolorbar:invalidLocation', ...
            'Valid location values are ''vertical'' and ''horizontal''.')
end

switch coloralignment
    case{'center','ends'}
    otherwise
        error('map:lcolorbar:invalidColorAlignment', ...
            'Valid coloralignment values are ''center'' and ''ends''.')
end
...
if objtest == 0
	error('map:lcolorbar:notAnAxes', ...
        'Invalid object type, h is not an axes object.')
end	
			...
			case 'ends'
				...
				if isempty(labels) == 0
					...
					if numberlabels ~= numberrequired
						warning('map:lcolorbar:invalidLabel',...
                         'Incorrect number of label entries: %s required to match colormap size', num2str(numberrequired))
					end
					yticklabels = labels;
				end	
