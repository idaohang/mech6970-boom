...
% $Revision: 1.1.6.3 $  $Date: 2009/12/02 06:43:45 $

% Validate maplonlimit input.
if ischar(maplonlimit) || (numel(maplonlimit) ~= 2 && ~isempty(maplonlimit))
    error(['map:' mfilename ':mapdispError'], ...
        'Incorrect %s property.', '''MapLonLimit''')
end
...
if okToSetMapLimits(mstruct,'MapLonLimit')
    ...
    if ~mprojIsAzimuthal(mstruct.mapprojection)
        ...
        elseif ~isequal(maplonlimit, mstruct.origin(2) + mstruct.flonlimit)
            warning('map:setMapLonLimit:maplonlimitMismatch', ...
                ['Ignoring the value provided for ''%s''.', ...
                ' If you are trying to use the ''%s'' property to', ...
                ' initialize the equatorial aspect of an azimuthal', ...
                ' projection, you should precede it with %s', ...
                ' in your call to %s.'], 'MapLonLimit', 'MapLonLimit', ...
                '''FLatLimit'',[]', 'AXESM')
        end
    end
