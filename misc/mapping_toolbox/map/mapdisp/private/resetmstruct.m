...
% $Revision: 1.1.6.1 $  $Date: 2008/05/14 22:01:30 $

%  Ensure that a map projection is specified
if isempty(mstruct.mapprojection)
    error(['map:' mfilename ':mapprojError'], 'No projection specified.')
end
...
% Defaults in degrees
if strmatch(zone,'north')
    maplatlimit    = [84 90];
    flatlimit      = [-Inf 6];
    origin         = [90 0 0];
    mlinelimit     = [84 89];
    mlabelparallel = 84;
elseif strmatch(zone,'south')
    maplatlimit    = [-90 -80];
    flatlimit      = [-Inf 10];
    origin         = [-90 0 0];
    mlinelimit     = [-89 -80];
...
defaults = struct(...
    'zone',           zone, ...
    'geoid',          almanac('earth','international','m'), ...
