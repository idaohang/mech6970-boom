...
%   graphics properties of the lines in the graticule.
%
...
% $Revision: 1.9.4.11 $  $Date: 2010/02/08 22:36:45 $
...
elseif rem(nargin,2) == 0
    ...
    error(['map:' mfilename ':incorrectNumberOfInputs'], ...
        'Incorrect number of arguments in function %s.', mfilename)
end
...
[lstyle,lcolor] = parseLineSpec(linespec);
props = {};
if ~isempty(lcolor)
    props{end+1} = 'GColor';
    props{end+1} = lcolor;
...
function [lstyle, lcolor, lmark] = parseLineSpec(linespec)
% Decode a LINESPEC string, returning the LineStyle property value in
% LSTYLE, the Color property value in LCOLOR, and the Marker property
% value in LMARK.  Issue an ERROR if LINESPEC is invalid.

[lstyle,lcolor,lmark,err] = colstyle(linespec);
if ~isempty(err)
...
%  If mstruct.galtitude is Inf, place the graticule
%  above the current ZLim setting.
alt = mstruct.galtitude;
if isinf(alt)
    alt = max(get(gca,'Zlim'))+1;
end
...
% Trim and project the graticule lines
projName = mstruct.mapprojection;
if strcmpi(projName,'globe')
    % Special case:  Globe's signature is different.
    alt = alt + zeros(size(lat));
    [x, y, z] = globe(mstruct, lat, lon, alt, 'none', 'forward');
else
    % "Regular" projection function.
