...
% $Revision: 1.8.4.8 $  $Date: 2008/10/26 14:26:55 $
...
for i = 1:length(h)
    ...
    if ~strcmp(get(parent,'Type'),'axes');
        error(['map:' mfilename ':mapdispError'], ...
            'Only children of axes may be projected')
    end
    gcm(parent);
    ...
    if ~(strcmp(type,'line') || strcmp(type,'surface') || ...
        ...
        warning('map:project:cannotProjectObject',...
            'Object can not be projected:  %s',type)
