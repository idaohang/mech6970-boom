...
% $Revision: 1.9.4.6 $  $Date: 2010/03/04 16:22:43 $
...
case 'initialize'
    if ~isempty(hndl) && (~isscalar(hndl) || ~ishghandle(hndl,'axes'))
        error(['map:' mfilename ':mapdispError'], ...
            'Input argument must be  an axes handle')
    else
         ...
         switch lower(fcn)
             case 'matrix'      %  Matrix search
                 ...
                 elseif length(cellarray) == 4
                     ...
                     result = 'Incorrect MATRIX cell array';
                     warning(['map:' mfilename ':invalidMatrixQuery'], result)
                 end
             ...
             case 'vector'      %  Indexed vector search
                 if length(cellarray) == 4
                     ...
                     if indx > 0 && indx <= numel(cellarray{4})
                         ...
                         result = 'Index exceeds array length';
                         warning(['map:' mfilename ':invalidIndexedVectorQuery'],result)
                     end
                 else
                     result = 'Incorrect VECTOR cell array';
                     warning(['map:' mfilename ':invalidVectorQuery'],result)
                 end
             ...
             case 'mapvector'      %  Indexed map vector search
                 if length(cellarray) == 3
                     ...
                     if indx > 0 && indx <= numel(cellarray{3})
                         ...
                         result = 'Index exceeds array length';
                         warning(['map:' mfilename ':invalidIndexedMapVectorQuery'],result)
                     end
                 else
                     result = 'Incorrect MAPVECTOR cell array';
                     warning(['map:' mfilename ':invalidMapVectorQuery'],result)
                 end
             ...
             case 'mapmatrix'      %  Map matrix search
                 ...
                 elseif length(cellarray) == 3
                     ...
                     result = 'Incorrect MAPMATRIX cell array';
                     warning(['map:' mfilename ':invalidMapMatrixQuery'],result)
                 end
             ...
             otherwise           %  User defined function search
                 if exist(fcn,'file') == 2
                     ...
                     warning(['map:' mfilename ':invalidFunctionQuery'],result)
                 end
         end
