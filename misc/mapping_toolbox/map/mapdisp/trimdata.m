...
% $Revision: 1.8.4.6 $  $Date: 2009/03/30 23:39:20 $
...
switch object
    ...
    case 'text'
         offset = degtorad(5);     %  Allow text to be 5 deg outside trim limits
		 xlim = [min(xlim)-offset max(xlim)+offset];
		 ylim = [min(ylim)-offset max(ylim)+offset];
		 [ymat,xmat,trimpts] = trimnans(ymat,ylim,xmat,xlim);

    case 'light'
         offset = degtorad(5);     %  Allow light to be 5 deg outside trim limits
		 xlim = [min(xlim)-offset max(xlim)+offset];
		 ylim = [min(ylim)-offset max(ylim)+offset];
		 [ymat,xmat,trimpts] = trimnans(ymat,ylim,xmat,xlim);
...
if ndims(xmat) > 2 || ndims(ymat) > 2
    error(['map:' mfilename ':mapdispError'], ...
        'Pages not allowed for xmat or ymat')
elseif ~isequal(size(xmat),size(ymat))
    error(['map:' mfilename ':mapdispError'], ...
        'Inconsistent xmat and ymat dimensions')
end
