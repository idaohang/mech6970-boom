...
% $Revision: 1.6.4.6 $  $Date: 2009/03/30 23:39:14 $
% Written by L. Job
...
case 'reconstruct'		
	s = get(gco,'userdata');
	if ishghandle(s.hsector); delete(s.hsector); end
	if ishghandle(s.hcenter); delete(s.hcenter); end
	if ishghandle(s.hcaz);    delete(s.hcaz);    end
	if ishghandle(s.hresize); delete(s.hresize); end
	if ishghandle(s.hrotate); delete(s.hrotate); end
	if ishghandle(s.hradius); delete(s.hradius); end
	% face 1
	[lat1,lon1] = track2(s.trk,s.clat,s.clon,s.slat,s.slon,[1 0],...
						 'deg',s.npts/3);
	% face 2
...
case 'centerdown'	
	...
	if ~ishghandle(s.hcenter) || ~ishghandle(s.hresize) || ~ishghandle(s.hradius) || ~ishghandle(s.hrotate) || ~ishghandle(s.hcaz)
		% code from trackui
