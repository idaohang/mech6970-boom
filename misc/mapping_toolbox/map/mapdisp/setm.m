...
% $Revision: 1.13.4.22 $  $Date: 2010/06/26 04:57:28 $
...
if max(size(varargin{1})) ~= 1 || ~ishghandle(varargin{1})
   error(['map:' mfilename ':mapdispError'], ...
       'Valid object handle required')
end
...
switch get(varargin{1},'Type')
    ...
    otherwise
        ...
        switch tagstr
            ...
            otherwise
                error(['map:' mfilename ':mapdispError'], ...
                    'Object type not supported by SETM.  Use SET.')
        end
end
...
if nargin < 2 || (nargin == 2 && ~isstruct(varargin{2}))
    error(['map:' mfilename 'tooFewInputs'], ...
...
ax = varargin{1};
if ~ismap(ax)
    error(['map:' mfilename ':axesNotMapped'], ...
        'Axis handle must refer to a map')
end
...
if reproQ
   ...
   for i = 1:numel(children)
      ...
      elseif ismapped(hChild)
         ...
         switch object
            case 'hggroup'
                error(['map:' mfilename ':hggroupHandleNotReprojectable'], ...
                    'The data in the hggroup cannot be reprojected.')
            ...
            otherwise
               ...
               %  New projection
               [x,y,z,savepts] = mfwdtran(newstruct,lat,lon,alt,object);  
         end
...
if reproQ
   ...
   for k = 1:numel(children)
      ...
      if ishghandle(h,'hggroup') && ~ismapped(h) && isempty(mapgraph)
          error(['map:' mfilename ':hggroupHandleNotReprojectable'], ...
              'The axes contain %s object data which cannot be reprojected.', ...
              'hggroup');
      end
   end
end
...
% verfied. The inputs will be verified with a call to AXESM.
...
if ~ismapped(varargin{1})
   error(['map:' mfilename ':mapdispError'], ...
       'Text object not projected.  Use SET')
end
...
if isempty(indx) || indx ~= 1
   error(['map:' mfilename ':mapdispError'], ...
       'Unrecognized property for Map Text')
end
...
if length(position) ~= 2 && length(position) ~= 3
   error(['map:' mfilename ':mapdispError'], ...
       'Position vector must be either 2 or 3 elements')
end
...
elseif nargin == 3
   ...
   if length(indx) ~= 1
      error(['map:' mfilename ':mapdispError'], ...
          'Unrecognized property for Surface Maps')
   end
...
if ~ismapped(varargin{1})
   error(['map:' mfilename ':mapdispError'], ...
       'Surface object not projected.  Use SET')
end
...
switch property
   ...
   case 'meshgrat'     %  New graticules for a regular matrix map
      ...
      if isempty(maplegend)
         error(['map:' mfilename ':mapdispError'], ...
             'MESHGRAT property requires maps displayed using MESHM')
      end
      ...
      if ~isempty(npts) && length(npts) ~= 2
         error(['map:' mfilename ':mapdispError'], ...
             'Two element npts vector must be supplied')
      end
   ...
   case 'graticule'     %  General map graticules
      ...
      if ndims(lat) > 2 || ndims(lon) > 2
         error(['map:' mfilename ':mapdispError'], ...
             'Pages not allowed for map graticules')
      elseif any(size(lat) ~= size(lon))
         error(['map:' mfilename ':mapdispError'], ...
             'Inconsistent graticule matrices')
      end
end
...
elseif ndims(alt) > 2 || any(size(lat) ~= size(alt))
   error(['map:' mfilename ':mapdispError'], ...
       'Inconsistent graticule and altitude matrices')
end
...
% Assign scale ruler handle
object = varargin{1};
hndl = findobj(object,'Type','hggroup');

% Get the properties structure
s = get(hndl,'Userdata');

% Handles of all elements of scale ruler
hchild = get(hndl,'Children');
...
for i=1:(nargin-1)/2
   ...
   if isempty(indx)
      error(['map:' mfilename ':mapdispError'],...
          'Unrecognized property for Scaleruler')
   elseif length(indx) > 1;
   ...
   switch MapProperties{indx}
      ...
      case 'Children'
         error(['map:' mfilename ':mapdispError'], ...
          'Attempt to modify read-only scaleruler property: ''Children''.')
   end

   % not much error checking yet. This may require cases for each property
   % Set the value
   s.(MapProperties{indx}) = varargin{3};
