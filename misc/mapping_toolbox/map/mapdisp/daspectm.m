...
%   any string recognized by UNITSRATIO.
%
...
% $Revision: 1.7.4.5 $  $Date: 2008/06/16 16:47:11 $
...
if strmatch(getm(gca,'mapprojection'),'globe')
   error(['map:' mfilename ':invalidAspectRatio'], ...
       'Aspect ratio changes not appropriate for Globe.')
end

% Cartesian coordinates of starting point
[xo,yo] = feval(mstruct.mapprojection,mstruct,lat,lon,'geopoint','forward');
if isempty(xo)
    warning(['map:' mfilename ':pointOutsideFrame'], ...
        ['The specified point, [%s], falls outside\nthe map frame.  ', ...
         'The frame center will be used instead.'], num2str([lat,lon]));
    [lat, lon] = frameCenter(mstruct);
    [xo,yo] = feval(mstruct.mapprojection,mstruct,lat,lon,'geopoint','forward');
end
