...
% $Revision: 1.14.4.9 $  $Date: 2010/03/22 03:52:16 $
...
if min(size(hndl)) ~= 1  || ndims(hndl) > 2  || any(~ishghandle(hndl))
   eid = sprintf('%s:%s:invalidHandle', getcomp, mfilename);
   error(eid, 'Vector of handles required.')

elseif max(size(hndl)) ~= 1 && ~isequal(size(hndl),size(zdata))
   eid = sprintf('%s:%s:inconsistentSize', getcomp, mfilename);
   error(eid, 'Inconsistent handles and zdata.')
end
...
if ishghandle(hndl(1),'hggroup')
   ...
   % Adjust the EdgeLine data if present.
   adjustEdgeLineData(hndl, zdata);
end
...
if numel(children) ~= numel(zdata)
   eid = sprintf('%s:%s:vectorFeaturesAndZDataMismatch', getcomp, mfilename);
   error(eid, ...
      'The number of elememnts in ZDATA does not match the number of vector features.')
end
...
% Adjust the EdgeLine data if present.
adjustEdgeLineData(children, zdata);

%--------------------------------------------------------------------------
function adjustEdgeLineData(hndl, zdata)
% Adjust the ZData in the EdgeLine handle (if present) in the HNDL array to
% ZDATA.

% Each patch created with mapshow and geoshow will has an associate line
% object that must also be updated.
polygonType = ishghandle(hndl(1),'patch') ...
    && any(strmatch('EdgeLine',fields(getappdata(hndl(1))),'exact'));
if polygonType
    if isscalar(zdata)
        % Shift all the edge lines into the same z = zdata plane.
        for hPatch = hndl(:)'
            hEdgeLine = getappdata(hPatch,'EdgeLine');
            sz = size(get(hEdgeLine,'XData'));
            set(hEdgeLine,'ZData',zdata + zeros(sz))
        end
    else
        % Shift each edge line into its own plane.
        for k = 1:numel(hndl)
            hPatch = hndl(k);
            hEdgeLine = getappdata(hPatch,'EdgeLine');
            sz = size(get(hEdgeLine,'XData'));
            set(hEdgeLine,'ZData',zdata(k) + zeros(sz))
        end
    end
end
...
for i = 1:length(hndl)
   ...
   if max(size(hndl)) == 1 && max(size(zdata)) ~= 1
      ...
      if all(size(zdata) == size(oldzdata))
         ...
         eid = sprintf('%s:%s:inconsistentZDataSize', getcomp, mfilename);
         error(eid, 'New zdata matrix different size from displayed zdata.')
      end
...
h.fig = dialog('Name','Specify Zdata',...
   'Units','Points',  ...
   'Position',PixelFactor*72*[2 1 3.5 1.7],...
   'Visible','off');
...
h.apply = uicontrol(h.fig, ...
   ...
   'Units','Points',  ...
   'Position', PixelFactor*72*[0.30  0.10  1.05  0.40], ...
   'FontWeight', 'bold', 'FontSize',FontScaling*10,...
...
h.cancel = uicontrol(h.fig, ...
   ...
   'Units','Points',  ...
   'Position',PixelFactor*72*[1.65  0.10  1.05  0.40], ...
   'FontWeight','bold',  'FontSize',FontScaling*10, ...
   'HorizontalAlignment','center', ...
   'ForegroundColor','black', 'BackgroundColor',figclr,...
   'CallBack','uiresume');
