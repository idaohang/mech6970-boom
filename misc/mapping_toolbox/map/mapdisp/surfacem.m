...
% $Revision: 1.15.4.7 $  $Date: 2009/05/14 17:06:28 $
...
if nargin == 0
    % Error on obsolete syntax SURFACEM
    errorOnMissingUI(mfilename)
elseif (nargin == 1) || (nargin == 2)
    % Error on obsolete syntaxes SURFACEM(Z) and SURFACEM(Z,GRATSIZE)
...
if any([ndims(lat) ndims(lon) ndims(alt)] > 2)
    error(['map:' mfilename ':mapdispError'], ...
        'Input lat, lon and alt matrices must not contain any pages.')
elseif ~isequal(size(lat),size(lon),size(alt))
    error(['map:' mfilename ':mapdispError'], ...
        'Inconsistent dimensions on input lat, lon and alt matrices.')
end
