...
%   surface distance units string 'sunits' can be any string recognized
%   by UNITSRATIO. The paper units string can be any dimensional units
%   string recognized for the figure PaperUnits property.
%       
...
%   If radius is a string, then it is evaluated as an ALMANAC body to
%   determine the spherical radius.  If numerical, it is the radius of
%   the desired sphere in the same units as the surface distance.  If
...
% $Revision: 1.8.4.7 $  $Date: 2009/03/09 19:16:32 $
...
if paperXdim > PaperSize(1) || paperYdim >  PaperSize(2)
	warning('map:paperscale:figureExceedsPaperSize',...
        'Figure is larger than paper size. Check page position with PAGEDLG.')
end
...
if strmatch(paperunits,'normalized'); 
	warning('map:paperscale:needAbsolutePaperUnits','Dimensional paper units required'); 
end
