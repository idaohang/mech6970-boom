...
%
%  See also GEOSHOW, MESHGRAT, MESHM, SURFACEM, SURFLM.

% Copyright 1996-2009 The MathWorks, Inc.
% $Revision: 1.9.4.7 $  $Date: 2009/05/14 17:06:30 $

if nargin == 0
    % Error on obsolete syntax SURFM
    errorOnMissingUI(mfilename)
elseif (nargin == 1) || (nargin == 2)
    % Error on obsolete syntaxes SURFM(Z) and SURFM(Z,GRATSIZE)
    errorOnObsoleteSurfmSyntax(mfilename,nargin)
end

latlimlonlimSyntax = (numel(lat) == 2) && (numel(lon) == 2);
if latlimlonlimSyntax
    % Ensure that size(lat) and size(lon) equal size(Z).  Without this
    % step, they'd be [50 100] instead.
    latlim = lat;
    lonlim = lon;
