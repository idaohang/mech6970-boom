...
%  $Revision: 1.16.4.10 $  $Date: 2009/12/28 04:17:11 $
...
case 'initialize'       %  Activate the tool
    mstruct = gcm(hndl);     %  Axes handle test performed here too.
...
case 'select'           %  Callback associated with the Mouse Select button
    ...
    mstruct = gcm(h.mapaxes);       %  convert it to degrees.
...
%  commands to the proper axes without activating the axes figure window.
    ...
    if get(h.twopt,'Value')   %  Two point track definition
        ...
        try
            ...
            errorflag = true;
            uiwait(errordlg(e.message,'Track Error','modal'))
        end
...
%  completed.  Now display the track(s).
    ...
    if isempty(otherprop)
        otherprop = [',''Tag'',',tagstr];
    else
        otherprop = [',',otherprop,',''Tag'',',tagstr];
    end
    try
...
case 'close'     %  Close Request Function;  Return to call fig if it exists
    h = get(get(0,'CurrentFigure'),'UserData');
	delete(h.figure)
    if ishghandle(h.mapaxes);   figure(get(h.mapaxes,'Parent'));   end
...
if ~ishghandle(h.figure);   return;   end
...
units = strvcat('Kilometers','Statute Miles','Nautical Miles','Radians');
