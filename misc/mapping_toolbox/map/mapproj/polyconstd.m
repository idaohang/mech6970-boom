function varargout = polyconstd(varargin)
%POLYCONSTD  Polyconic Projection -- Standard
%
%  POLYCONSTD  Implements the Polyconic projection directly on a
%  reference ellipsoid, consistent with the industry-standard definition
%  of this projection.  See POLYCON for an alternative implementation
...
% $Revision: 1.1.6.4 $  $Date: 2007/12/10 21:39:43 $

mproj.default = @polyconstdDefault;
mproj.forward = @polyconstdFwd;
mproj.inverse = @polyconstdInv;
mproj.auxiliaryLatitudeType = 'rectifying';
mproj.classCode = 'Pstd';

varargout = applyProjection(mproj, varargin{:});

%--------------------------------------------------------------------------

function mstruct = polyconstdDefault(mstruct)
...
function [x, y] = polyconstdFwd(mstruct, mu, lambda)

[a, e, M0, epsilon, radius] = deriveParameters(mstruct);
...
phi = convertlat(mstruct.geoid, mu, 'rectifying', 'geodetic', 'nocheck');
...
N = a ./ sqrt(1 - (e*sin(phi(~q))).^2);
E = lambda(~q) .* sin(phi(~q));
x(~q) = N .* cot(phi(~q)) .* sin(E);
y(~q) = radius*mu(~q) - M0 + N .* cot(phi(~q)) .* (1-cos(E));

%--------------------------------------------------------------------------

function [mu, lambda] = polyconstdInv(mstruct, x, y)

[a, e, M0, epsilon] = deriveParameters(mstruct);
...
mu = convertlat(mstruct.geoid, phiNew, 'geodetic', 'rectifying', 'nocheck');

%--------------------------------------------------------------------------

function [a, e, M0, epsilon, radius] = deriveParameters(mstruct)
...
M0 = meridianarc(0, phi0, mstruct.geoid);
