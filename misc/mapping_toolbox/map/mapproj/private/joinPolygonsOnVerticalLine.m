...
% $Revision: 1.1.6.3 $  $Date: 2007/02/11 05:47:36 $
...
if ~isempty(splitParts.(side).startsOnBoundary) ...
    ...
    zStart = xIn.(side)(splitParts.(side).iStart(looseStartIndx(:))) ...
        + i * yIn.(side)(splitParts.(side).iStart(looseStartIndx(:)));

    zEnd = xIn.(side)(splitParts.(side).iEnd(looseEndIndx(:)))...
        + i * yIn.(side)(splitParts.(side).iEnd(looseEndIndx(:)));
...
if ~isempty(partNumber)
    ...
    if numel(partNumber) > 1
        eid = sprintf('%s:%s:sharedEndPoint', getcomp, mfilename);
        error(eid,'Corrupt data: Two parts end at the same boundary point.');
    end
end
...
if ~isempty(partNumber)
    ...
    if numel(partNumber) > 1
        eid = sprintf('%s:%s:sharedStartPoint', getcomp, mfilename);
        error(eid,'Corrupt data: Two parts start at the same boundary point.');
    end
end
...
elseif any(unused.RIGHT)
    ...
    error(eid,'Internal error in subfunction locateStartOfOutputPart.');
end
