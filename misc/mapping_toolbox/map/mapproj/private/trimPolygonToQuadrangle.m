...
% $Revision: 1.1.8.12 $  $Date: 2010/06/07 16:34:23 $
...
if ~isempty(lat)
    ...
    [lon, lat] = closePolygonInRectangle( ...
        lon, lat, lonlim, [-pi/2 pi/2], tolSnap);
    ...
    [lonT, latT] = closePolygonInRectangle( ...
        lonT, latT, complement, [-pi/2 pi/2], tolSnap);
...
% last vertices are replicated down the rows. Put NaNs on the diagonal
% to eliminate identity matches.
D = diag(NaN + zeros(numel(first),1));
...
% Exclude pairs that connect at xlimit(1) or xlimit(2).
onEdge = (x(first2) == xlimit(1)) | (x(first2) == xlimit(2));
first1(onEdge) = [];
first2(onEdge) = [];
last1( onEdge) = [];
last2( onEdge) = [];
...
assert(...
    ...
    ['map:' mfilename ':connectionsChain'], ...
    ['Inconsistent polygon topology:', ...
    ' Encountered a chain of more than two connecting pairs.'])
