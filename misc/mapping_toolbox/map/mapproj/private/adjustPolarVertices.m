...
% $Revision: 1.1.6.3 $  $Date: 2009/10/16 05:00:37 $
...
if preTrimmed
    ...
    elseif numel(p) > 3
        error(['map:' mfilename ':tooManyPolarVertices'], ...
            '%d polar vertices: Too many.', numel(p))
    end
end
...
k = find(diff(p) > 1);
assert(numel(k) <= 1, ...
    ['map:' mfilename ':multipleVisitsToPole'], ...
    ['%d runs of polar vertices:', ...
    ' Polygon appears to make multiple visits to pole.'], numel(k))
...
elseif (p2 - p1) == 1
    ...
    error(['map:' mfilename ':invalidPairOfPolarVertices'], ...
        ['Two polar vertices:', ...
        ' Must either be adjacent or at opposite endpoints.'])
end
...
elseif isequal(p, [s e-1 e]')
    ...
    error(['map:' mfilename ':invalidTripletOfPolarVertices'], ...
        ['Three polar vertices: Must include', ...
        ' both endpoints and a vertex adjacent to an endpoint.'])
end
