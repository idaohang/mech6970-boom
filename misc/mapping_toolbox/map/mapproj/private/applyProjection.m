...
% $Revision: 1.1.8.12 $  $Date: 2010/03/22 03:52:21 $
...
q = ~ingeoquad(radtodeg(lat), radtodeg(lon), ...
    radtodeg(flatlimit), radtodeg(flonlimit));
lat(q) = [];
lon(q) = [];
...
function [lat, lon] = trimMeshToQuadrangle(lat, lon, flatlimit, flonlimit)
...
% Insert NaNs at the appropriate locations in the surface graticule mesh so
% that a displayed map is clipped at the appropriate edges.
...
switch  mstruct.aspect
    ...
    case 'transverse'
        ...
        out2 = real(-x);
    otherwise
        eid = sprintf('%s:%s:invalidProjectionAspect', getcomp, mfilename);
        error(eid, 'Unrecognized aspect string')
end

%--------------------------------------------------------------------------

function [out1, out2] = applyProjAspectTran(mstruct, x, y)

% Ignore projection aspect value for Transverse Mercator/UTM.
out1 = real(x);
out2 = real(y);
if ~strcmp(mstruct.aspect,'normal')
    wid = sprintf('%s:%s:ignoringProjectionAspect', getcomp, mfilename);
    warning(wid, 'Ignoring projection aspect for Transverse Mercator.')
end
...
if ~strcmp(mstruct.aspect,'normal')
    wid = sprintf('%s:%s:ignoringProjectionAspect', getcomp, mfilename);
    warning(wid, 'Ignoring projection aspect for Conic Projection.')
end
...
elseif strcmp(direction,'inverse')
    ...
    error(eid, 'Unrecognized DIRECTION string.')
end
