...
% $Revision: 1.5.4.3 $  $Date: 2007/10/10 20:50:04 $
% Written by A. Kim
...
elseif nargin == 1
	if isstr(varargin{1})
		...
		ll = varargin{1};
		if size(ll,2) ~= 2
			error(['map:' mfilename ':mapprojError'], 'Lat-long matrix must have two columns')
		end
		lat = ll(:,1);
		lon = ll(:,2);
		clear ll
...
if strcmp(findit,'zone')
	...
	if isempty(msg)
		...
		if nargout<2,  error(['map:' mfilename ':mapprojError'], msg);	 end
...
elseif strcmp(findit,'limits')
	...
	if nargout < 2
		if ~isempty(msg),  error(['map:' mfilename ':mapprojError'], msg),  end
