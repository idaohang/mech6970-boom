...
% $Revision: 1.1.6.8 $  $Date: 2008/05/14 22:01:49 $
...
if nargin > 1
    ...
    if ~strmatch(mstruct.aspect,'normal')
        wid = sprintf('%s:%s:ignoringNonNormalAspect', getcomp, mfilename);
        warning(wid,...
            'Aspect setting ''%s'' ignored by Transverse Mercator projection.',...
            mstruct.aspect);
        varargin{1} = mstruct;
    end
end
...
[scalefactor, ellipsoid, e2, phi0, lambda0, x0, y0] = deriveParameters(mstruct);
a = ellipsoid(1);
F0 = scalefactor;
...
function [scalefactor, ellipsoid, e2, phi0, lambda0, x0, y0] ...
    = deriveParameters(mstruct)

ellipsoid = mstruct.geoid;
e2 = (mstruct.geoid(2))^2;
