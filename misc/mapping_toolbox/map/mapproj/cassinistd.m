...
% $Revision: 1.1.6.4 $  $Date: 2007/03/27 19:13:36 $
...
if nargin > 1
    ...
    if ~strmatch(mstruct.aspect,'normal')
        ...
        warning(wid,...
            'Aspect setting ''%s'' ignored by Cassini projection.',...
            mstruct.aspect);
        varargin{1} = mstruct;
    end
end
...
if numel(mstruct.geoid) == 1 || mstruct.geoid(2) == 0
    % Spherical form
    ...
    % Ellipsoid form
    [a, e, e2, phi0] = deriveParameters(mstruct);
...
function [a, e, e2, phi0] = deriveParameters(mstruct)
