...
%   strings, either FROM or TO, indicates angular units and the other
%   unit string indicates length units, uses a great circle arc on a
%   sphere of the given radius.  The specified length units must apply
%   to RADIUS as well as to the input distance (when FROM indicates
%   length) or output distance (when TO indicates length).  If neither
%   FROM nor TO indicates angular units, or if both do, then the value
%   of RADIUS is ignored.
%
%   distOut = DISTDIM(distIn, FROM, TO, SPHERE), where either FROM or
%   TO indicates angular units, uses a great circle arc on a sphere
%   approximating a body in the Solar System.  SPHERE may be one of
%   the following strings: 'sun', 'moon', 'mercury', 'venus', 'earth',
%   'mars', 'jupiter', 'saturn', 'uranus', 'neptune', or 'pluto', and is
...
%   several other strings for each of these units; see the UNITSRATIO
...
% $Revision: 1.12.4.5 $  $Date: 2009/03/30 23:38:19 $
...
% Check the FROM and TO strings for supported units,
...
switch from
    case 'degrees'
        switch to
            ...
            case 'radians',           dist = degtorad(dist);
    ...
    case 'radians'
        switch to
            case 'degrees',           dist = radtodeg(dist);
