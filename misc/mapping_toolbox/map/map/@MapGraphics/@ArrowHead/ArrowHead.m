...
% $Revision: 1.1.6.7 $  $Date: 2008/11/24 14:59:03 $

ax = ancestor(hLine,'axes');
this = MapGraphics.ArrowHead;
this.line = hLine;
...
this.hPatch = patch(...
    'Parent',ax,'XData',xt,'YData',yt,'FaceColor',c, ...
    'EdgeColor','None','Visible',get(this.line,'Visible'),...
    'HandleVisibility','off','DeleteFcn',@deleteArrowHead);
...
set(toolButton,'State','off');
    ...
    function setVisible(hSrc,event)  %#ok<INUSD>
        set(this.hPatch,'Visible',get(this.line,'Visible'))
    end

    function setColor(hSrc,event)  %#ok<INUSD>
        set(this.hPatch,'FaceColor',get(this.line,'Color'))
    end

    function moveArrow(hSrc,event)  %#ok<INUSD>
