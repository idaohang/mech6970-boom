...
%   GeoRasterReference object.  R may be any valid referencing vector,
%   as long as the cell size 1/R(1), northwest corner latitude R(2), and
%   northwest corner longitude R(3) lead to valid latitude and longitude
%   limits when combined with the rasterSize vector. Alternatively, R
%   may be any valid referencing matrix subject to the constraints that
%   (1) it leads to valid latitude and longitude limits when combined
%   with rasterSize and (2) its columns and rows are aligned with
%   meridians and parallels, respectively. The latter condition is true
%   only when the 1,1 and 2,2 elements of R vanish. Finally, R may
%   already be a valid GeoRasterReference object, with a
%   RasterInterpretation of 'cells' and with RasterSize and AngleUnits
%   properties that are consistent with the 2nd and 3rd inputs passed to
%   this function.

% Copyright 2009-2010 The MathWorks, Inc.
% $Date: 2010/10/11 14:47:31 $  $Revision: 1.1.6.5 $
...
elseif isequal(size(R), [3 2])
    ...
    assert( ...
        validGeoRasterReference(R, rasterSize, angleUnits), ...
        ...
        ' or a (scalar) spatialref.GeoRasterReference object.'], ...
        upper(func_name), arg_pos, var_name_R)
end

%------------------------------------------------------------------------

function tf = validGeoRasterReference(R, rasterSize, angleUnits)
% Return true if R has RasterSize, and AngleUnits properties that are
...
if tf
    ...
    assert(strcmp(R_angleUnits, angleUnits), ...
       'map:convertToGeoRasterRef:angleUnitsMismatch', ...
       '%s is inconsistent with ''%s.''', 'R.AngleUnits', angleUnits)
end
