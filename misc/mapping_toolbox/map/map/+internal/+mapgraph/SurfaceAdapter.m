...
% $Revision: 1.1.6.1 $  $Date: 2010/03/04 16:22:26 $

classdef SurfaceAdapter < hgsetget
    ...
    methods
        ...
        function delete(h)
        % Delete the associated surface handles.
            for n=numel(h.SurfaceHandle):-1:1
                if ~isempty(h.SurfaceHandle(n)) ...
                        && ishghandle(h.SurfaceHandle(n), 'surface')
                    delete(h.SurfaceHandle(n))
                end
            end
        end
    ...
    methods (Access = protected, Hidden = true)
        ...
        function S = updateProperties(h, S) %#ok<MANU>
        ...
        % property names.
            ...
            S = get(h);
            names = properties(h);
            index = ismember(names, h.NamesOfSurfaceProps);
    ...
    methods (Access = private)
        ...
        % properties; otherwise create a new surface object.
            ...
            if hasValidSurfaceHandle(h)
                ...
                % Set a DeleteFcn callback such that when the surface
                % handle is deleted, the object h is also deleted.
                set(h.SurfaceHandle,  'DeleteFcn',  @deleteFunction)
            end
