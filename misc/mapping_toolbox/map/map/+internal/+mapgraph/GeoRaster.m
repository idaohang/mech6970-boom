...
% $Revision: 1.1.6.5 $  $Date: 2010/11/17 11:24:40 $
...
classdef GeoRaster < internal.mapgraph.SurfaceAdapter
    ...
    methods
        ...
        % the current properties of h.
            ...
            onCleanup(@() set(h,'ListenersEnabled', true));
    ...
    methods (Access = private)
        ...
        % properties listed in varargin.
            ...
            names = names(ismember(names, fieldnames(get(surfaceHandle))));
...
% surface ('2d'), then use pixedges; otherwise if it is a 3D surface, then
% use pixcenters.
if strcmpi(displayType, '2d')
    [lon, lat] = pixedges(R, size(Z));
else
...
% mstruct.
[S.XData, S.YData, Z] = mfwdtran(mstruct, lat, lon, S.ZData, 'geosurface');
...
%PIXEDGES Compute pixel edges for georeferenced image or data grid
%
...
default = [];
parent = internal.map.findNameValuePair('Parent', default, varargin{:});
