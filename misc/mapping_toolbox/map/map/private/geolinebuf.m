function [latb, lonb] = geolinebuf(lat, lon, bufwidth, n)
...
% $Revision: 1.1.6.2 $  $Date: 2010/06/07 16:34:08 $
...
isolatedPoints = first((first == last));
if ~isempty(isolatedPoints)
    for k = isolatedPoints'
        [phib, lambdab] = geopointbuf(lat(k), lon(k), cx, cy, cz);
        [lata, lona] = maptrimp( ...
            radtodeg(phib), radtodeg(lambdab), latlim, lonlim);
        % Work around the fact that polybool won't accept empty inputs.
        if isempty(latb)
            latb = lata;
            lonb = lona;
        else
            [lonb, latb] = polybool('union', lona, lata, lonb, latb);
        end
    end
end
...
for j = 1:numel(first)
    for k = first(j):(last(j)-1);
        [phib, lambdab] = geoarcbuf(lat(k), lon(k), lat(k+1), lon(k+1), ...
            degtorad(bufwidth), cxEast, cyEast, czEast, cxWest, cyWest, czWest);
        [lata, lona] = maptrimp( ...
            radtodeg(phib), radtodeg(lambdab), latlim, lonlim);
        % Work around the fact that polybool won't accept empty inputs.
        if isempty(latb)
