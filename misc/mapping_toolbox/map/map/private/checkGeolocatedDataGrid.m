...
% $Revision: 1.1.6.1 $  $Date: 2010/03/22 03:51:54 $

% Validate coordinate arrays.
attributes = {'real', 'nonempty', '2d'};
checkinput(c1,'numeric', attributes, fcnName, rules.C1, rules.posC1C2(1));
checkinput(c2,'numeric', attributes, fcnName, rules.C2, rules.posC1C2(2));

% Validate Z.
if require2D
    checkinput(Z, 'numeric', attributes, fcnName, 'Z', 3);
    checkMatrixSizes(c1, c2, size(Z), rules.isGeoCoord);
else
    checkinput(Z, 'numeric', {'real', 'nonempty'}, fcnName, 'Z', 3);
