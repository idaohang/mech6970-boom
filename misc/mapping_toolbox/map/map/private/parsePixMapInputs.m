...
%   $Revision: 1.1.6.4 $  $Date: 2010/11/17 11:24:55 $
...
if ~isfield(info,'RefMatrix') || ~isfield(info,'Height') || ~isfield(info,'Width')
    ...
    R = info.RefMatrix;
    try
        internal.map.validateRasterReference(R, {}, func_name, 'R', 1)
    catch %#ok<CTCH>
...
internal.map.validateRasterReference(R, ...
    {'spatialref.MapRasterReference'}, func_name, 'R', 1)
...
if isobject(R)
    ...
    assert(isequal(height, rasterSize(1)) && isequal(width, rasterSize(2)), ...
        ...
        'Function %s expected the %s property of the %s object to be consistent with its raster size inputs (%s or %s and %s).', ...
        func_name, 'RasterSize', 'spatialref.MapRasterReference', ...
        'SIZEA', 'HEIGHT', 'WIDTH')
end
