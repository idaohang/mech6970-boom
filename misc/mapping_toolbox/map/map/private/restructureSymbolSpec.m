...
% $Revision: 1.1.6.1 $  $Date: 2006/05/24 03:35:49 $
    ...
    for j = 1:numel(newspec.Properties(k).Rules)
        ...
        if any(strcmp(rule.AttributeName, attributeNames))
            ...
            if ~strcmp(rule.AttributeClass, attributeClasses{n});
                eid = sprintf('%s:%s:inconsistentAttributeClasses', ...
                              getcomp, mfilename);
                error(eid, 'Symbolspec contains inconsistent attribute classes: %s and %s', ...
...
%
% An interplotingMap returns a cell array with a scalar double in each
...
if ischar(attributeValueOrRange)
    ...
    elseif matchRange
        ...
        elseif isColormap(propertyValueOrRangeOrColormap)
            ...
            map = setupColorMappingMap(...
                specifiedAttributeRange, colormap);
        elseif (numel(propertyValueOrRangeOrColormap) == 2)
            ...
            eid = sprintf('%s:%s:invalidPropertyValueOrRange', ...
                getcomp, mfilename);
            error(eid, 'Symbol spec property specification had %d elements; it must be\na one or two-element vector, a 1-by-3 color, or a colormap array.', ...
                numel(propertyValueOrRangeOrColormap))
        end
    else
        % Note: We should never reach this line because makesymbolspec
        % should error first.
        eid = sprintf('%s:%s:invalidAttributeSpecification', ...
            getcomp, mfilename);
        error(eid, 'Symbol spec attribute specification had %d elements;\nit must be a one or two-element vector.', ...
...
function map = setupColorMappingMap(attributeRange, colormap)

map = @colorMappingMap;

    function [index, propertyValues] = colorMappingMap(attributeValues)
        index = (attributeValues >= attributeRange(1)) ...
              & (attributeValues <= attributeRange(2));
        if any(index)
            p = interp1(attributeRange, ...
                        [1 size(colormap,1)], attributeValues(index));
        else
            p = [];
        end
        propertyValues = cell(numel(p),1);
        for k = 1:numel(p)
            propertyValues{k,1} = colormap(round(p(k)),:);
        end
    end
end
