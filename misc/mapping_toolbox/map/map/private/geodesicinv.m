...
%   ------------------------
%   There is a small but finite input space, consisting of pairs in which
%   both the points are nearly antipodal AND both points fall close to
%   (but not precisely on) the Equator.  In this case, a warning is
%   issued and S, AZ, and BAZ are set to NaN for the "problem pairs."
%
%   The behavior in this case differs from the original NGS program (see
%   reference below) in two ways:  (1) We have corrected a logic error
%   which, given a pair of points that were nearly 180 degrees apart in
%   longitude, could result in failure to obtain a valid solution even
%   if one of the points was "not too close" to the Equator.  (2) In the
%   event that a solution could not be found, the output values are set to
%   NaN rather than zero.

%   Copyright 2004-2009 The MathWorks, Inc.
%   $Revision: 1.1.4.3 $  $Date: 2009/11/09 16:26:00 $
...
validateattributes(phi2,      supportedClasses, requiredAttributes, mfilename)
validateattributes(lambda2,   supportedClasses, requiredAttributes, mfilename)
