...
%attributes2properties Rule-based graphic properties for geostruct/mapstruct
%
%   PROPERTIES = attributes2properties(SYMSPEC, S) applies the
%   symbolization rules in SYMSPEC to the attribute values in the
%   geographic data structure array, S.  SYMSPEC is a symbol
%   specification structure returned by MAKESYMBOLSPEC. S is a geostruct
%   or mapstruct returned by SHAPEREAD, for example.  The return value,
%   PROPERTIES, is a structure that can be passed to the function
%   EXTRACTPROPS, such that
%
...
% $Revision: 1.1.6.4 $  $Date: 2008/06/16 16:46:56 $
...
missingAttributes = setdiff({symspec.Attributes.Name},fields(S));
if ~isempty(missingAttributes) && ~isequal(missingAttributes,{'INDEX'})
    eid = sprintf('%s:%s:missingAttributes', getcomp, mfilename);
    error(eid, ['symspec specifies attributes that are missing from' ,...
        ' the geographic data structure S.'])
end
...
for m = 1:numel(symspec.Attributes)
    ...
    if strcmp(attributeName,'INDEX')
        ...
        if strcmp(symspec.Attributes(m).Class,'char')
            % String-valued attributes
            attributeValues.(attributeName) ...
                = getStringValues(S, attributeName);
        else
            % Numeric scalar-valued attributes
            attributeValues.(attributeName) ...
                = getNumericValues(S, attributeName);
        end
    end
end
...
properties.Values = cell(numel(S), numel(properties.Names));
...
if ~isfield(S, 'Geometry')
    ...
    error(eid,'Geographic data structure S must have a ''Geometry'' field.')
end
...
if any(~cellfun(@ischar,geometryArray))
    ...
    error(eid,...
        ['An element of geographic data structure S has a non-string', ...
        ' ''Geometry'' field.'])
end
...
if ~any(strcmp(geometry,validGeometries))
    ...
    error(eid, ...
        ...
        ' must contain one of the following strings: %s, %s, %s, %s.'],...
        validGeometries{:})
end
...
if ~(pointMatch || lineMatch || polygonMatch)
    eid = sprintf('%s:%s:mismatchedGeometry',getcomp,mfilename);
    error(eid, ...
        ['Geographic data structure geometry %s is inconsistent', ...
        ' with symbolspec geometry %s.'], geometry, shapeType);
end
...
try
    ...
    if strcmp(exception.identifier,'MATLAB:nomem')
        ...
        error([getcomp ':' mfilename ':inconsistentAttributeData'], ...
...
if ~isa(v,'double')
    error([getcomp ':' mfilename ':nondoubleNumericAttribute'], ...
        ['Attribute field %s of geographic data structure S contains', ...
        ' at least one non-double value.'], attributeName)
...
if any(~cellfun(@ischar,v))
    eid = sprintf('%s:%s:inconsistentCharacterClass', getcomp, mfilename);
    error(eid,...
