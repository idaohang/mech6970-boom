...
% $Revision: 1.1.6.4 $  $Date: 2010/06/26 04:57:14 $
...
if computePolygons
    ...
    if ~isempty(xNull)
        ...
        for k = 1:numel(P)
            [P(k).X, P(k).Y] = polybool( ...
                'subtraction', P(k).X, P(k).Y, xNull, yNull);
        end
    end
else
    % Adjust the line topology only.
...
function [L, P] = fixTopology(L, xLimit, yLimit, xmax, ymax)
% Update contour line topology and generate topological contour
% polygons. Starting with the output of contourc as encoded in line
% mapstruct L, adjust the line topology of each level as required to
% ensure that the "uphill" area falls on the right as one traverses the
% vertices from first to last. Then construct a polygon mapstruct P,
% closing curves along the boundary of the rectangle defined by the
% intervals xLimit and yLimit.

% Initialize polygon mapstruct.
P(1 + numel(L),1) = struct(...
    'Geometry', [], ...
    'BoundingBox', [], ...
    'X', [], ...
    'Y', [], ...
...
% Initialize lowest contour to bounding rectangle.
x1 = xb;
y1 = yb;

% Update contour lines and assign P(1) through P(end-1).
for k = 1:numel(L)
    % Adjust topology for lines (in x, y) and close up open curves
    % to form a valid polygon topology (in xp, yp).
    [x, y, xp, yp] = adjustContourTopology( ...
        L(k).X, L(k).Y, xLimit, yLimit, xmax, ymax);
    % Subtract the polygon bounded by the current contour from the one
    % at the level immediately below it (using vertices stored in x1,
    % y1).  The various polygons will not intersect in the interior of
    % the rectangle, but they might intersect along the edges. To cover
    % all cases, use polybool to perform the subtractions (and densify
    % later as needed).
    [xd, yd] = polybool('subtraction', x1, y1, xp, yp);
    
    % Add this "difference" polygon to the geostruct P.
    P(k).Geometry = 'Polygon';
    P(k).BoundingBox = [min(xd) min(yd); max(xd) max(yd)];
    P(k).X = xd;
    P(k).Y = yd;
    
    % Save polygon vertices to use when processing next level up.
    x1 = xp;
    y1 = yp;
    
    % Update geostruct L with topologically valid lines.
    L(k).X = x;
    L(k).Y = y;
    L(k).BoundingBox = [min(x) min(y); max(x) max(y)];
end

% Set the mapstruct fields for the uppermost polygon. There is no
% level above this one, so there's nothing to subtract; use x1 and y1
% directly.
P(end).Geometry = 'Polygon';
P(end).BoundingBox = [min(x1) min(y1); max(x1) max(y1)];
P(end).X = x1;
P(end).Y = y1;
