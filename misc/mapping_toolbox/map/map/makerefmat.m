...
%   ColumnsStartFrom 
%
%      ColumnsStartFrom is a string indicating the column direction of
%      the raster (south-to-north vs. north-to-south) in terms of the
%      edge from which row indexing starts. It can have the value
%      'south' or 'north'. The default value is 'south', suiting the
%      typical terrain grid in which row indexing starts at the
%      southern edge. In images, on the other hand, row indexing starts
%      typically at the northern edge. The input string can be shortened
%      and is case-insensitive.
%
%   RowsStartFrom
%
%      RowsStartFrom is a string indicating the row direction of the
%      raster (west-to-east vs. east-to-west) in terms of the edge from
%      which column indexing starts. It can have the value: 'west' or
%      'east'. The default value is 'west', indicating rows that run
%      from west to east, which almost always the case. The input
%      string can be shortened and is case-insensitive.
%
...
%      % -90 degrees and extending to +90 degrees, for a given longitude.
...
%      % by using LATLON2PIX with the referencing matrix:
%
%      [row, col] = latlon2pix(geoidR, 4.5, 78.5)
%      geoid(round(row),round(col))
%
%   See also GEORASTERREF, MAPRASTERREF, WORLDFILEREAD, WORLDFILEWRITE

% Copyright 1996-2010 The MathWorks, Inc.
% $Revision: 1.1.8.10 $  $Date: 2010/11/17 11:24:22 $
...
%
% See worldFileMatrixToRefmat for more information.
%--------------------------------------------------------------------
...
if nargin == 0 || ischar(varargin{1})
    ...
    switch(nargin)
        case 1
            R = worldFileMatrixToRefmat(varargin{1});
        case 4
            R = worldFileMatrixToRefmat(constructW(varargin{:}));
        otherwise
...
% Verify that inputs are string-value pairs.
...
% Construct a default GeoRasterReference object.
R = georasterref('RasterSize',[1 1], 'Latlim', [0 1], 'Lonlim', [0 1]);
...
% to set the value of the corresponding GeoRasterReference property.
validParameters = {'RasterSize', ...
...
refmat = convertToReferencingMatrix(R);
