...
% $Revision: 1.9.4.7 $  $Date: 2009/03/30 23:38:32 $
...
elseif ~isequal(size(casetype),size(lat))
	error(['map:' mfilename ':mapError'], ...
        'casetype vector must be empty or same size as latitude input')
end
...
if length(az)~=length([jndx(:);indx(:)])
	error(['map:' mfilename ':mapError'], 'casetype must be 0 or 1')
end
...
for i=1:size(pair,1)
	if casetype(pair(i,1))==1
		if casetype(pair(i,2))==1  % This is the casetype where both objects are azimuths (rhumb lines)
			...
			% slope of line
			slope=tan(pi/2 - degtorad(az(pair(i,1))));
			if isinf(slope)
				intercpt=linex;
			else
	...
	elseif	casetype(pair(i,2))==1   % This is the casetype with the first a range, second an azimuth
		...
		% slope of line
		slope=tan(pi/2 - degtorad(az(pair(i,2))));
		if isinf(slope)
				intercpt=linex;
			else
				intercpt=liney-slope*linex;
...
if ~isempty(drlat)
	...
	if any(isnan(newlat))
		newlat=[];    newlong=[];
		warning('map:navfix:noFix','No fix.')
	end
end
