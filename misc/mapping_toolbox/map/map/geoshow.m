...
%   be set to 'image'.
%
%   GEOSHOW(S) or GEOSHOW(S, ..., 'SymbolSpec', SYMSPEC, ...) displays
...
%   the 'Geometry' field of S.  If S is a geostruct (with 'Lat' and
%   'Lon' coordinate fields) vertices are projected to map coordinates.
%   If S is a mapstruct (with 'X' and 'Y fields), vertices are plotted
%   as (pre-projected) map coordinates and a warning is issued. SYMSPEC
%   specifies the symbolization rules used for the vector data through a
%   structure returned by MAKESYMBOLSPEC.
%
...
%   the case of polygons, a modified patch object.  If a geostruct or
%   shapefile name is input, GEOSHOW returns the handle to an hggroup
%   object with one child per feature in the geostruct or shapefile,
%   excluding any features that are completely trimmed away.  In the case
%   of a polygon geostruct or shapefile, each child is a modified patch
%   object;  otherwise it is a line object.
%
...
% $Revision: 1.1.6.12.2.1 $  $Date: 2011/01/29 14:47:31 $
