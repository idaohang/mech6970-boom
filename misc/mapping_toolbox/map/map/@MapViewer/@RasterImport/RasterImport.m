...
%   Only variables that are 3x2 are valid referencing matrices.
%   Only scalar variables with class spatialref.MapRasterReference are
%   valid referencing objects.

% Copyright 1996-2010 The MathWorks, Inc.
% $Revision: 1.1.6.11 $  $Date: 2010/11/17 11:24:50 $
...
if isempty(refName)
  errordlg('The image must be referenced by a 3-by-2 numeric matrix or a scalar spatialref.MapRasterReference object.','Import Error','modal');
  status = FAIL;
  return;
else
...
if ~(isnumeric(R) || (isscalar(R) && isa(R, 'spatialref.MapRasterReference')))
  errordlg('The image must be referenced by a 3-by-2 numeric matrix or a scalar spatialref.MapRasterReference object.','Import Error','modal');
  status = FAIL;
  return
...
% If necessary, convert R from a spatialref.MapRasterReference object to
...
% or a scalar spatialref.MapRasterReference object.
for i=1:numel(workspaceVars)
  v = workspaceVars(i);
  if isequal(v.size, [3 2]) || (isequal(v.size, [1 1]) ...
          && strcmp(v.class, 'spatialref.MapRasterReference'))
    vars(j) = v;
    j = j + 1;
  end
end
