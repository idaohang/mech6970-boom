...
%   where appropriate.
%
%   See also earthRadius.

% Copyright 1996-2010 The MathWorks, Inc.
% $Revision: 1.1.6.8 $  $Date: 2010/09/24 14:33:04 $
    ...
    switch parm
	  case 'radius'
		...
		warning('map:almanac:usingSemiMajorAxis', ...
            'Semimajor axis returned for radius parameter.')
	  case 'airy'
        mat = airy1849;    mat(1) = km2dist(mat(1),units,'earth');
	  case {'geoid','ellipsoid'}
        mat = airy1849;    mat(1) = km2dist(mat(1),units,'earth');
	  case 'volume'
        fact = km2dist(1.0,units,'earth');
...
case 'wgs84'
    switch parm
      ...
      case 'volume'
        fact = km2dist(1.0,units,'earth');
        mat = elpcalc(wgs84,parm);      mat = mat * fact^3;
      case 'surfarea'
        fact = km2dist(1.0,units,'earth');
        mat = elpcalc(wgs84,parm);      mat = mat * fact^2;
    end
...
otherwise
    error('map:almanac:invalidRefBody','Unrecognized reference body string.')
end
