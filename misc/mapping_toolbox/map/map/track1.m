...
% $Revision: 1.10.4.5 $  $Date: 2007/11/09 20:25:42 $
...
if ~isequal(size(lat),size(lon),size(az))
    error(['map:' mfilename ':mapError'], ...
        'Inconsistent dimensions for lat,long, and azimuth inputs.')
elseif ndims(lat) > 2 || size(lat,2) ~= 1
    error(['map:' mfilename ':mapError'], ...
        'Lat,long, and azimuth inputs must be column vectors.')
...
if rhumbdefault  %  Default limits for rhumb line calculation.
    ...
    if geoid(1) == 0
        ...
        rec = convertlat(geoid, lat, 'geodetic', 'rectifying', 'nocheck');
    end
    negcolat = pi/2+rec;
    poscolat = pi/2-rec;
