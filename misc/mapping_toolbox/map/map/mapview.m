...
%                              viewer but does not add it to your
%                              workspace.
%
%     Import From Workspace    Select referencing matrix or object and
%     -> Raster Data -> Image  data representing the image from the list
%                              of workspace variables. If the image type
%                              is true color (RGB), specify which band
...
%     -> Geographic Data       Normally, this would be an mapstruct, but
%        Structure             if a geostruct is supplied, then
%                              latitudes are substituted for Y and
%                              longitudes for X.
%
...
%   PROJINV or MINVTRAN, then re-project with PROJFWD or MFWDTRAN.  You
%   might also need to adjust the horizontal datum of your data using, for
%   example, the free GEOTRANS (Geographic Translator) application from the
%   Geospatial Sciences Division of the U.S. National Geospatial-
%   Intelligence Agency (NGA).  If you simply need a change of units,
%   multiply by the appropriate conversion factor obtained from UNITSRATIO.
%
...
%   symbolspecs in a MAT-file (or save calls to MAKESYMBOLSPEC in a
%   MATLAB script or function).
%
...
%                   follows, select a different attribute, or "INDEX."  If
...
%                   value of a single attribute.  If you need to compare two
...
%   supported by modern web browsers.  The export process automatically
%   creates a world file (ending with suffix 'tfw', 'jgw', or 'pgw') as
%   well; the pair of files constitute a georeferenced image that itself
%   can be displayed with MAPVIEW, MAPSHOW, and many external GIS packages. 
...
% $Revision: 1.1.6.11 $  $Date: 2010/11/17 11:24:27 $
