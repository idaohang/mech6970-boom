...
%   property must be consistent with size(Z) and its
...
%   referencing matrix, then R_TRIMMED is a referencing matrix. If R is
%   a spatialref.GeoRasterReference object, then R_TRIMMED is either a
%   spatialref.GeoRasterReference object (when Z_TRIMMED is non-empty)
%   or [] (when Z_TRIMMED is empty).
%
%   See also MAPTRIML, MAPTRIMP.

% Copyright 1996-2010 The MathWorks, Inc.
% $Revision: 1.11.4.14 $  $Date: 2010/11/17 11:24:26 $

error(nargchk(4, 5, nargin, 'struct'))
Z = ignoreComplex(Z, mfilename, 'Z');
validateattributes(Z, {'double'}, {'2d','real'}, 'maptrims', 'Z', 1)
...
if (nargin == 4)
    ...
    rowDensity = abs(S.DeltaLatDenominator / S.DeltaLatNumerator);
    colDensity = abs(S.DeltaLonDenominator / S.DeltaLonNumerator);
...
if all(rasterSize > 0)
    ...
    if isobject(R)
        ...
        Rtrimmed = convertToReferencingMatrix(S);
    end
else
    % Empty output raster.
    if isobject(R)
        ...
        dlat = rowSampleFactor * S.DeltaLatNumerator / S.DeltaLatDenominator;
        dlon = colSampleFactor * S.DeltaLonNumerator / S.DeltaLonDenominator;
        Rtrimmed = makerefmat( ...
            lonlim(1 + (1 - sign(S.DeltaLon))/2) + dlon/2, ...
            latlim(1 + (1 - sign(S.DeltaLat))/2) + dlat/2, ...
            dlon, dlat);
    end
end
...
latlim = snapLatlim(S, rlim, yiLim);
lonlim = snapLonlim(S, clim, xiLim);
...
if all(rasterSize > 0)
    ...
    if decreasingDensity
        % Call resizem but don't ask it to manage spatial referencing.
        Ztrimmed = resizem(Ztrimmed, rasterSize, 'nearest');
    end
else
    % Empty output raster; avoid calling resizem.
...
function latlim = snapLatlim(S, rlim, yiLim)
...
function lonlim = snapLonlim(S, clim, xiLim)
