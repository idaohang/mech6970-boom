...
%   elements of the binary regular data grid BW which contain the value
%   1 (true).  BW might be the result of a logical expression such as
%   BW = (topo > 0).  R can be a spatialref.GeoRasterReference object, a
%   referencing vector, or a referencing matrix.
%
%   If R is a spatialref.GeoRasterReference object, its RasterSize
%   property must be consistent with size(BW) and its
...
%   ranges from 0 to 1.
%
%   A = AREAMAT(BW, R, ELLIPSOID) uses the input ELLIPSOID vector
%   to describe the sphere or reference ellipsoid.  ELLIPSOID has the
%   form [semi-major-axis-length, eccentricity].  The units of the
%   output, A, are the square of the length units in which the
%   semi-major axis is provided.  For example, if ELLIPSOID is replaced
%   with almanac('earth','wgs84','kilometers'), then A is in square
%   kilometers.
%
...
% $Revision: 1.9.4.9 $  $Date: 2010/11/17 11:24:12 $
