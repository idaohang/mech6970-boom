...
%
%   See also: GEOSHOW, MAPSHOW, MAPVIEW, UPDATEGEOSTRUCT.

% Copyright 1996-2008 The MathWorks, Inc.
% $Revision: 1.1.6.7 $  $Date: 2008/06/16 16:46:46 $
...
for i=1:length(varargin)
  ...
  if ischar(rule{1}) && strcmpi(rule{1},'default')
    ...
    if numel(rule) < 4
      ...
      msg = sprintf('%s%i%s\n%s','RULE number ',i,' is invalid.', ...
            'An Attribute Name/Value pair must precede a Property Name/Value pair');
      error(eid, '%s', msg);
    end
