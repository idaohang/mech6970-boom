function [trackRes] = akos_tracking(fileid, acq, stop_time)
  ...
  for k = 1:n_code_per
    ...
    if ~rem(k,50)
      waitbar((k/n_code_per), hwb, ...
        ['(Akos) Tracking Channel ' num2str(ch) '/' num2str(acq.nsv) ' - PRN' num2str(acq.svs(ch)) ' : ' num2str(k) '/' num2str(n_code_per)]);
    end
